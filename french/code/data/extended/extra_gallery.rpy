translate french strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Lydia"
    new "Jour 1: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ignore the girl Pete is sleeping with (name's Hana)"
    new "Ignore la fille Pete couche avec (nom de Hana)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "Jour 1: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "Jour 2: Message de Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Did you post your lips in the morning?"
    new "As-tu poster tes lèvres le matin?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Jour 2: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "Jour 2: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Déverrouillé en n’affichant aucune demande sur FL ou en ignorant la réponse"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "Jour 3: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Vous devez avoir fait jouir Min durant le jour 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Fête: Daisy et Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Choisissez l’événement Père noeïl secret suceur"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Fête: La Fille mystérieuse"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "Jour 4: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "Jour 4: La baignoire d'AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Polly"
    new "Jour 4: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "Jour 5: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "Jour 5: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Vous devez découvrir le secret de Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "Jour 5: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "Jour 5: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Donner des cours particuliers à Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Stacy"
    new "Jour 6: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "Jour 6: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Nora"
    new "Jour 6: Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Ne pas remplir Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "Jour 6: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Tu as dû choisir Père noeïl secret suceur mais tu ne connais pas encore son secret"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "Jour 6: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "Vous avez vu toutes les images possibles que Maria peut envoyer"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "Jour 7: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "Jour 7: beach"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Hana"
    new "Jour 7: Hana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Ne demandez pas la sortie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Daisy"
    new "Jour 7: Daisy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Lyssa"
    new "Jour 7: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "Jour 7: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: AmRose"
    new "Jour 7: AmRose"

translate french strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "Stacy en enfer"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Décevez Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Bonus d'Halloween - Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Halloween Spécial"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Bonus d'Halloween - Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Bonus d'Halloween - Samiya"

