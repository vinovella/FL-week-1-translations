# TODO: Translation updated at 2020-07-05 14:03

translate russian strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "День 1: Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "День 2: сообщение Сквизли"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Day 2: Сквизли"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "День 2: Полли"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Разблокировать, не отправляя запрос в ФЛ или проигнорировав ответ"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "День 3: Мин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Заставить Мин кончить на 2-й день"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Вечеринка: Дейзи и Далия"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Выбрать событие Секретный Оральный Санта"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Вечеринка: Таинственная девушка"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "День 4: Лидия"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "День 4: ванна АмРоуз"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Polly"
    new "День 4: Полли"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "День 5: Ванесса"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "День 5: Лисса"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Раскрыть тайну Лиссы"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "День 5: Джесси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "День 5: Мин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Дайте Мин частные уроки"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Stacy"
    new "День 6: Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "День 6: Карен"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Nora"
    new "День 6: Нора"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Не кончать в Нору"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "День 6: Лисса I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Выбрать Секретного Орального Санту, не зная ее секрет"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "День 6: Лисса II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "Увидеть все возможные фотографии, которые Мария может отправить"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "День 7: Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "День 7: Пляж"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Hana"
    new "День 7: Ханой"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Не просить об освобождении яиц"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Daisy"
    new "День 7: Дейзи"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Lyssa"
    new "День 7: Лисса"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "День 7: Полли"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: AmRose"
    new "День 7: АмРоуз"

# TODO: Translation updated at 2021-01-18 23:56

translate russian strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "Стейси в аду"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Разочаровать Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Бонус на Хэллоуин - Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Специальный выпуск к Хэллоуину"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Бонус на Хэллоуин - Джесси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Бонус на Хэллоуин - Самия"

