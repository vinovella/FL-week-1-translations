# game/code/d02/s06.rpy:18
translate chinese d02s06_d141bee8:

    # "Day 2"
    "第二天"

# game/code/d02/s06.rpy:21
translate chinese d02s06_52759d10:

    # "7:00 AM"
    "早上七点"

# game/code/d02/s06.rpy:24
translate chinese d02s06_df6d5b35:

    # mc "Good morning me!"
    mc "早上好！"

# game/code/d02/s06.rpy:25
translate chinese d02s06_e99e848f:

    # mc "Seems like Pete never came back home last night. Probably found a good party or two."
    mc "看来皮特昨晚没回来。可能还办了一两个派对呢。"

# game/code/d02/s06.rpy:27
translate chinese d02s06_36d55616:

    # mc "({i}But I had a nice time with AmRose yesterday as well.{/i})"
    mc "({i}不过昨天我和爱洛丝玩得也很开心。{/i})"

# game/code/d02/s06.rpy:29
translate chinese d02s06_5e8dc635:

    # mct "I feel excited for today!"
    mct "我今天充满干劲！"

# game/code/d02/s06.rpy:30
translate chinese d02s06_2a2bc255:

    # mc "({i}Let`s take a shower and go to the studies.{/i})"
    mc "({i}先洗个澡，然后去学习吧。{/i})"

# game/code/d02/s06.rpy:39
translate chinese d02s06_4ee8cc8f:

    # "9:00 AM"
    "上午九点"

# game/code/d02/s06.rpy:48
translate chinese d02s06_b3ad3221:

    # mct "There is AmRose!"
    mct "是爱洛丝！"

# game/code/d02/s06.rpy:54
translate chinese d02s06_f11ce5d8:

    # mc "Hi AmRose! I have to go ask Kevin about some things! I'll see you at the next lecture!"
    mc "嗨，爱洛丝！有些事我得去问问凯文！我们下节课见！"

# game/code/d02/s06.rpy:58
translate chinese d02s06_7a1090b9:

    # arj "Sure! See you later!"
    arj "好啊！回头见！"

# game/code/d02/s06.rpy:62
translate chinese d02s06_4cd44184:

    # arj "..."
    arj "……"

# game/code/d02/s06.rpy:63
translate chinese d02s06_89124156:

    # arj "({i}Oh I forgot that I will have to go home soon. I will have to skip the next lecture.{/i})"
    arj "({i}呀，我忘了自己待会就要回家了。我需要逃掉下一节课来着。{/i})"

# game/code/d02/s06.rpy:64
translate chinese d02s06_bc2f9008:

    # arjt "I hope [mcname] won't be upset with me because of that."
    arjt "我希望[mcname]不会因为这件事而生我的气。"

# game/code/d02/s06.rpy:68
translate chinese d02s06_0cbf64f8:

    # mc "({i}Seems like I managed to upset AmRose. She seems pissed at me.{/i})"
    mc "({i}我好像惹恼了爱洛丝啊。她好像生我的气了。{/i})"

# game/code/d02/s06.rpy:73
translate chinese d02s06_d7bc8020:

    # mc "Hey! Morning."
    mc "嘿，早啊。"

# game/code/d02/s06.rpy:75
translate chinese d02s06_47f1da4a:

    # kb "Hey back. Got any punishments today?"
    kb "欢迎回来。今天有受什么惩罚吗？"

# game/code/d02/s06.rpy:77
translate chinese d02s06_448253f2:

    # mc "Not yet! But there is plenty of time today!"
    mc "还没有！但今天还长着呢！"

# game/code/d02/s06.rpy:79
translate chinese d02s06_c1e70be4:

    # kb "Ahah! Right!"
    kb "哈哈！对啊！"

# game/code/d02/s06.rpy:83
translate chinese d02s06_fa7b3d1b:

    # mc "Mr Nordin is not here yet. Is he late?"
    mc "诺丁先生还没来。他迟到了吗？"

# game/code/d02/s06.rpy:85
translate chinese d02s06_8b96f65f:

    # kb "Yeah. Maybe he won't come at all?"
    kb "对啊。也许他根本就不来了？"

# game/code/d02/s06.rpy:87
translate chinese d02s06_050c649e:

    # mc "That would be good. I could use some free time."
    mc "那太好了。我就有更多空闲时间了。"

# game/code/d02/s06.rpy:90
translate chinese d02s06_5f4b12f4:

    # kb "How did the cleaning go?"
    kb "你打扫工作做得怎么样了？"

# game/code/d02/s06.rpy:93
translate chinese d02s06_fa8c5876:

    # mc "It wasn’t that bad! Pete joined and we finished everything faster!"
    mc "不算糟糕！有皮特帮忙，打扫就好办多了！"

# game/code/d02/s06.rpy:95
translate chinese d02s06_80dcb2f0:

    # mc "We cleaned everything pretty quickly!"
    mc "我们很快就把所有东西都搞定了！"

# game/code/d02/s06.rpy:97
translate chinese d02s06_ebdccf1b:

    # kb "Ah nice!"
    kb "啊不错哎！"

# game/code/d02/s06.rpy:100
translate chinese d02s06_0f9e6cc9:

    # kb "How did the meeting with Pete go?"
    kb "你和皮特见面怎么样了？"

# game/code/d02/s06.rpy:102
translate chinese d02s06_beb56350:

    # mc "Nice! Pete also showed me Fetish Locator!"
    mc "很好啊！皮特还给我看了性趣探测器！"

# game/code/d02/s06.rpy:104
translate chinese d02s06_fd005d89:

    # kb "I see! I told you! Almost everyone at our college is using it!"
    kb "看吧！我早跟你说了！我们学校几乎每个人都在用它！"

# game/code/d02/s06.rpy:106
translate chinese d02s06_b16ae0ae:

    # mc "({i}Heh. I probably shouldn't tell him about his [game.chloekevin] sucking off Pete just to get points in the app!{/i})"
    mc "({i}呵。我也许不应该告诉他他[game.chloekevin]为了拿软件的点数给皮特口的事！{/i})"

# game/code/d02/s06.rpy:108
translate chinese d02s06_a9e74211:

    # mc "({i}I even have the picture of Chloe covered in cum! Definitely should keep that to myself!{/i})"
    mc "({i}我甚至还有克洛伊满身精液的照片！绝对要保密！{/i})"

# game/code/d02/s06.rpy:109
translate chinese d02s06_673ade22:

    # mc "Pete told me about the party! Tickets are sold in the app. But I need to get 50 points to get there!"
    mc "皮特跟我说了派对的事！在软件里可以买到票。但我需要赚50点数才能买！"

# game/code/d02/s06.rpy:111
translate chinese d02s06_52b42f0d:

    # kb "Wow, that seems like a lot!"
    kb "哇，需要这么多！"

# game/code/d02/s06.rpy:112
translate chinese d02s06_abb7e839:

    # kb "Does it have anything to do with Min and Antony? You told me they were talking about some party yesterday."
    kb "这个跟明还有安东尼有什么关系吗？昨天你说他们在讨论什么派对。"

# game/code/d02/s06.rpy:114
translate chinese d02s06_bcb169b5:

    # mc "Oh! You’re right! I completely forgot about that! It might be the same party! Maybe Min and Antony are organizing this party?"
    mc "啊！没错！我都忘了这回事！可能就是同一个派对呢！也许是明和安东尼在组织这个派对？"

# game/code/d02/s06.rpy:116
translate chinese d02s06_8f16e9c2:

    # kb "Why not?"
    kb "可能哦。"

# game/code/d02/s06.rpy:118
translate chinese d02s06_c8708a17:

    # mc "Maybe I could find something out."
    mc "也许我能再找点什么线索。"

# game/code/d02/s06.rpy:120
translate chinese d02s06_1982d67d:

    # kb "And why did Pete tell you about this party? I don't remember him taking you to any parties."
    kb "话说皮特为什么告诉你这个派对？我不记得他带你参加过什么派对啊。"

# game/code/d02/s06.rpy:122
translate chinese d02s06_48aa700c:

    # mc "Oh, yeah. I forgot to tell you. He is sure that Lydia will be there."
    mc "啊，我忘了告诉你。他说莉迪亚肯定会去的。"

# game/code/d02/s06.rpy:124
translate chinese d02s06_d04e2ad1:

    # kb "Ah! I see. Lydia! That explains a lot. So 50 points than? How do you plan to get them?"
    kb "哈，明白了。莉迪亚！这就说得通了。所以你打算怎么拿50点数？"

# game/code/d02/s06.rpy:126
translate chinese d02s06_034cedef:

    # mc "I don't have a real plan yet."
    mc "我连计划都还没有呢。"

# game/code/d02/s06.rpy:128
translate chinese d02s06_f63fa37c:

    # mc "But I already got some points, and I hope to get more today and tomorrow!"
    mc "但我已经得到了一些点数，希望今天和明天能拿更多！"

# game/code/d02/s06.rpy:130
translate chinese d02s06_7033deac:

    # kb "Hmmm I see. Well, good luck with that!"
    kb "这样啊。好吧，祝你好运！"

# game/code/d02/s06.rpy:132
translate chinese d02s06_6d04953c:

    # mc "Thanks!"
    mc "谢啦！"

# game/code/d02/s06.rpy:140
translate chinese d02s06_56672fe1:

    # fl "Thanks. Your user name is saved now."
    fl "谢谢参与。你的用户名已保存。"

# game/code/d02/s06.rpy:142
translate chinese d02s06_181f9e16:

    # fl "Yesterday was a lot of fun! But boys had a huge advantage! It is so much easier for you to get some cum!"
    fl "昨天真是太有趣了！不过男生有很大的优势！你们要想得到精液容易太多了！"

# game/code/d02/s06.rpy:144
translate chinese d02s06_03c5dbf6:

    # fl "Today will be about girls! Fetish of the day is ..."
    fl "今天的主题是女生！今日性趣是……"

# game/code/d02/s06.rpy:146
translate chinese d02s06_e52bd013:

    # fl "Go and eat some pussy!"
    fl "捕食小穴！"

# game/code/d02/s06.rpy:147
translate chinese d02s06_45b7e47a:

    # mc "({i}Oh fuck!{/i})"
    mc "({i}卧槽！{/i})"

# game/code/d02/s06.rpy:149
translate chinese d02s06_c10b8745:

    # mc "({i}Now I'm regretting not putting 100%% of effort yesterday!{/i})"
    mc "({i}现在我是真后悔昨天没有付出100%%的努力！{/i})"

# game/code/d02/s06.rpy:151
translate chinese d02s06_0c8dfce3:

    # mct "I should have tried to earn more points yesterday!"
    mct "我昨天就应该多赚些点数的！"

# game/code/d02/s06.rpy:152
translate chinese d02s06_e0b96f3c:

    # mc "({i}How am I going to find a girl for that?!{/i})"
    mc "({i}这让我怎么去找妹子?!{/i})"

# game/code/d02/s06.rpy:154
translate chinese d02s06_a4b4692d:

    # mc "({i}It is not like I can lick a girl's pussy without her knowing, like I did yesterday cumming on AmRose.{/i})"
    mc "({i}我又不能像昨天在爱洛丝身上射精那样，在女孩不知情的情况下舔她的小穴。{/i})"

# game/code/d02/s06.rpy:156
translate chinese d02s06_3da94ecc:

    # mc "({i}That was so crazy! I came on AmRose while she was sleeping!{/i})"
    mc "({i}这太疯狂了！我是趁爱洛丝睡着的时候射的！{/i})"

# game/code/d02/s06.rpy:157
translate chinese d02s06_9edc2817:

    # mct "What if she knew about it? How would she react?"
    mct "如果她知道了怎么办？她会有什么反应？"

# game/code/d02/s06.rpy:170
translate chinese d02s06_f2a7f36b:

    # mct "Min just left the class?"
    mct "明刚是离开教室了？"

# game/code/d02/s06.rpy:171
translate chinese d02s06_d854af83:

    # mct "It wasn't 15 minutes yet. Teacher might still come."
    mct "这上课还没到15分钟呢。老师可能还是会来。"

# game/code/d02/s06.rpy:174
translate chinese d02s06_45a656e0:

    # "Couple minutes later"
    "几分钟后"

# game/code/d02/s06.rpy:177
translate chinese d02s06_015d20de:

    # mct "Wow!"
    mct "哇哦！"

# game/code/d02/s06.rpy:178
translate chinese d02s06_39d50247:

    # mct "She came back in a skirt! She did it right after today`s fetish was announced!"
    mct "她穿着短裙回来了！她在今日性趣公布后才这样的！"

# game/code/d02/s06.rpy:179
translate chinese d02s06_2d7431a4:

    # mct "I already know {i}what{/i} she and Antony are planning to do!"
    mct "我已经知道她和安东尼打算做{i}什么{/i}了！"

# game/code/d02/s06.rpy:186
translate chinese d02s06_fd7af61c:

    # mc "And here is Mr. Nordin. Lecture is still going to happen."
    mc "诺丁先生来了。还是要上课的。"

# game/code/d02/s06.rpy:188
translate chinese d02s06_ab1437c2:

    # kb "Yep! Sadly!"
    kb "没错！真惨！"

# game/code/d02/s06.rpy:191
translate chinese d02s06_f1c2fe01:

    # mc "Man! This fetish of the day is a bummer!"
    mc "伙计！这个今日性趣真是够操蛋！"

# game/code/d02/s06.rpy:193
translate chinese d02s06_d32089cb:

    # kb "Yea, you need to find a girl for this!"
    kb "可不是，你得找一个女孩才行！"

# game/code/d02/s06.rpy:195
translate chinese d02s06_a985b678:

    # kb "There is an option to buy a rubber pussy in a sexshop, but I doubt you will get any points for that!"
    kb "你也可以选择在情趣用品店里买一个假阴部，不过我怀疑这样有没有点数！"

# game/code/d02/s06.rpy:197
translate chinese d02s06_ec080e62:

    # mc "Haha! Yeah!"
    mc "哈哈！是啊！"

# game/code/d02/s06.rpy:198
translate chinese d02s06_03a1bc66:

    # mc "({i}But do I have a better idea?!{/i})"
    mc "({i}不过我还有什么更好的法子吗?!{/i})"

# game/code/d02/s06.rpy:200
translate chinese d02s06_0b3da5ed:

    # kb "Or you can put an ad on the app."
    kb "或者你也可以在软件上投个广告。"

# game/code/d02/s06.rpy:202
translate chinese d02s06_7a0e86b4:

    # mc "What is that?"
    mc "怎么说？"

# game/code/d02/s06.rpy:204
translate chinese d02s06_c9dc21ba:

    # kb "You pay some money and post an ad in the app with the info what you are looking for! Maybe you'll get lucky!"
    kb "你付点钱，在软件上发个广告，写上你需要的信息！也许你就走运了！"

# game/code/d02/s06.rpy:206
translate chinese d02s06_e7baac27:

    # mc "Let's see how to do that..."
    mc "我看看怎么搞啊……"

# game/code/d02/s06.rpy:208
translate chinese d02s06_2514821a:

    # mc "It costs $5 to put an ad up for one day! That’s almost robbery!"
    mc "投一天广告要5刀！这不是抢劫嘛！"

# game/code/d02/s06.rpy:210
translate chinese d02s06_de13e7e8:

    # kb "Well that or the rubber pussy. and the rubber pussy will cost you more!"
    kb "要不就去买假阴道吧，只不过这个价格更高！"

# game/code/d02/s06.rpy:212
translate chinese d02s06_7cbebcce:

    # mc "That is true!"
    mc "那倒是！"

# game/code/d02/s06.rpy:214
translate chinese d02s06_6ef48d10:

    # kb "You need to post a picture with an ad."
    kb "你的广告上要贴照片。"

# game/code/d02/s06.rpy:216
translate chinese d02s06_428e5365:

    # mc "Picture of what??"
    mc "什么照片??"

# game/code/d02/s06.rpy:218
translate chinese d02s06_8a9895aa:

    # kb "Well, it would be a dickpic if that is what you offering. But in this case you are actually going to use a different part of the body!"
    kb "好吧，如果你实际真要这样那就太蠢了。但在这种情况下，你难道不该把身体部位用起来，好吸引人吗！"

# game/code/d02/s06.rpy:220
translate chinese d02s06_c153789c:

    # mc "So should I post a photo of my tongue?!"
    mc "那我是该放一张我舌头的照片?!"

# game/code/d02/s06.rpy:222
translate chinese d02s06_2a312316:

    # kb "Well that or your lips! Think for yourself which will work better?"
    kb "可以，或者是你的嘴唇！你自己想想哪个效果更好？"

# game/code/d02/s06.rpy:224
translate chinese d02s06_4c3165dc:

    # mc "Actually lips sounds better!"
    mc " 好像嘴唇好一点！"

# game/code/d02/s06.rpy:226
translate chinese d02s06_67f28a29:

    # mc "({i}But should I post this at all? This whole idea sounds weird.{/i})"
    mc "({i}但我到底该不该发呢？这整个主意听起来都很奇怪。{/i})"

# game/code/d02/s06.rpy:227
translate chinese d02s06_ac0e2301:

    # mct "What if someone would find this out? This is a bit scary."
    mct "万一被人发现了怎么办？有点吓人啊。"

# game/code/d02/s06.rpy:228
translate chinese d02s06_e12dc444:

    # mct "So should I do it?"
    mct "那我要不要做呢？"

# game/code/d02/s06.rpy:233
translate chinese d02s06_9e0f00f0:

    # mc "Okay, let's do this!"
    mc "好，就这么办吧！"

# game/code/d02/s06.rpy:240
translate chinese d02s06_eda69769:

    # mc "Done!"
    mc "搞定！"

# game/code/d02/s06.rpy:242
translate chinese d02s06_0cb958e7:

    # kb "Cool!"
    kb " 酷！"

# game/code/d02/s06.rpy:244
translate chinese d02s06_1db01c34:

    # mc "So how long should I wait?"
    mc "所以我要等多久啊？"

# game/code/d02/s06.rpy:246
translate chinese d02s06_fed0d51d:

    # kb "How do I know?! I never posted anything!"
    kb "我怎么知道?!我又没发过广告！"

# game/code/d02/s06.rpy:248
translate chinese d02s06_1e782752:

    # mc "What?! And why would you tell me to do that if you’ve never done it yourself?!"
    mc "什么?!你自己都没做过，为什么要叫我做?!"

# game/code/d02/s06.rpy:250
translate chinese d02s06_13e82622:

    # kb "You wanted to know how to find someone with a pussy! I just told you what I know!"
    kb "是你想知道怎么找愿意给你小穴的人！我就把我知道的告诉你了！"

# game/code/d02/s06.rpy:252
translate chinese d02s06_1c641253:

    # mc "{i}Great{/i}! Okay, we’ll see how this works out!"
    mc "{i}呵{/i}！好吧，我们看看这个到底怎么回事！"

# game/code/d02/s06.rpy:256
translate chinese d02s06_30d1c34f:

    # mc "No, that’s too weird! I’ll try something else!"
    mc "不行，这太奇怪了！我再试试别的办法！"

# game/code/d02/s06.rpy:262
translate chinese d02s06_cbc032a3:

    # fl "You have a new message!"
    fl "你有新消息了！"

# game/code/d02/s06.rpy:263
translate chinese d02s06_feebcc28:

    # "Anon" "Meet me on the bench in the park behind the old campus in 30 minutes. Have a banana in your hand!"
    "匿名" "30分钟后在老校区后面公园的长椅上见。手里拿根香蕉！"

# game/code/d02/s06.rpy:264
translate chinese d02s06_18c1cbff:

    # mc "({i}Wow! That simple?! But I don't even know who that is! What if it is not even a girl?{/i})"
    mc "({i}哇，这么简单吗?!可是我都不知道那是谁啊！如果来的人连女生都不是呢？{/i})"

# game/code/d02/s06.rpy:265
translate chinese d02s06_e654802c:

    # mc "({i}And what the fuck is this with banana? I hope it is just to recognize me from a random person...{/i})"
    mc "({i}还有，拿香蕉是要搞什么？希望这只是为了从人群中认出我……{/i})"

# game/code/d02/s06.rpy:266
translate chinese d02s06_c1d164ba:

    # mc "({i}That looks a bit scary. Should I respond to this?{/i})"
    mc "({i}这看起来有点吓人。我要不要回应呢？{/i})"

# game/code/d02/s06.rpy:270
translate chinese d02s06_03051b13:

    # mc "That is totally insane! No way I'm going there!"
    mc "太疯狂了！我不可能去那里！"

# game/code/d02/s06.rpy:277
translate chinese d02s06_7a8abb2b:

    # mc "({i}Should I tell Kevin about this? But what if this message is from his [game.chloekevin]? She is using this app very actively!{/i})"
    mc "({i}我应该把这件事告诉凯文吗？但如果这条信息是他的[game.chloekevin]发来的呢？她可是很热衷于玩这个软件呢！{/i})"

# game/code/d02/s06.rpy:278
translate chinese d02s06_c6450146:

    # mc "({i}I better keep it to myself.{/i})"
    mc "({i}我最好保密。{/i})"

# game/code/d02/s06.rpy:280
translate chinese d02s06_700c9a78:

    # mc "({i}It shouldn't be dangerous to sit on a bench in the park! If I don't like what is offered then I can just leave!{/i})"
    mc "({i}坐在公园的长椅上应该不会有什么危险吧！如果我不喜欢对方的方案，直接离开就是！{/i})"

# game/code/d02/s06.rpy:281
translate chinese d02s06_3aba2a79:

    # mc "({i}Okay - so I guess I should confirm that I agree. Meet you in 30 minutes.{/i})"
    mc "({i}好吧--所以我想我应该是同意。30分钟后见。{/i})"

# game/code/d02/s06.rpy:282
translate chinese d02s06_bbd8c845:

    # "[mcname] is sending the message in the Fetish Locator app."
    "[mcname]正在使用性趣探测器软件发送消息。"

# game/code/d02/s06.rpy:284
translate chinese d02s06_23b63442:

    # mc "({i}Done! Interesting. Who could it be?{/i})"
    mc "({i}好了！有意思。会是谁呢？{/i})"

# game/code/d02/s06.rpy:285
translate chinese d02s06_5ff75b01:

    # mct "This message is sent by someone from our college. This is a good chance it is someone I know!"
    mct "这个消息是我们学校的人发来的。很有可能是我认识的人！"

# game/code/d02/s06.rpy:288
translate chinese d02s06_b619ca43:

    # mct "Could that be AmRose? She knows about the app. Is she using it?"
    mct "难道是爱洛丝？她是知道这个软件的。她有在用吗？"

# game/code/d02/s06.rpy:289
translate chinese d02s06_b3d32ba0:

    # mct "How should I react if I meet her in the park?"
    mct "如果在公园里遇到她，我该怎么回应啊？"

# game/code/d02/s06.rpy:290
translate chinese d02s06_0ce90a01:

    # mct "Would she be Okay with me eating her out?! Would I be good with that? It would definitely change our friendship."
    mct "她能接受我碰她下面吗?!我又能接受吗？我们的友谊肯定会变味儿。"

# game/code/d02/s06.rpy:292
translate chinese d02s06_83790a4e:

    # mct "Where can I find a banana now?"
    mct "现在哪里可以找到香蕉？"

# game/code/d02/s06.rpy:293
translate chinese d02s06_00db0acd:

    # mc "({i}I should sneak out of the lecture and go to the cafeteria to look for one.{/i})"
    mc "({i}我应该溜出教室，去食堂找一根。{/i})"

# game/code/d02/s06.rpy:296
translate chinese d02s06_9db38bfb:

    # rn "Okay, let`s have a 15 minute break!"
    rn "好，我们休息15分钟吧！"

# game/code/d02/s06.rpy:298
translate chinese d02s06_4b30f8fa:

    # mct "This is my chance!"
    mct "机会来了！"

# game/code/d02/s06.rpy:301
translate chinese d02s06_cdf56ffd:

    # mct "I need to find a way to get points!"
    mct "我得想办法拿点数！"

# game/code/d02/s06.rpy:302
translate chinese d02s06_06c64dd8:

    # mct "Sitting here won't get me any points..."
    mct "坐在这里也不会有点数的……"

# game/code/d02/s06.rpy:305
translate chinese d02s06_618be91c:

    # mc "Hey I will go now, super hungry and the lecture is boring!"
    mc "嘿，我先走了，肚子好饿，这课又这么无聊！"

# game/code/d02/s06.rpy:307
translate chinese d02s06_a972c0f2:

    # kb "See you!"
    kb "拜拜！"

# game/code/d02/s06.rpy:309
translate chinese d02s06_bf9f08f4:

    # "[mcname] is leaving the class, but on the way out he remembers about Min changing into skirt."
    "[mcname]正要离开教室，但在出去的路上，他想起了明换裙子的事。"

# game/code/d02/s06.rpy:311
translate chinese d02s06_71eab1da:

    # mct "Damn, I will miss seeing Antony eat Min out! Will they do it in class again?!"
    mct "妈的，我会错过安东尼舔明下面！他们还会在课堂上搞吗?!"

# game/code/d02/s06.rpy:312
translate chinese d02s06_e13fab6e:

    # mct "Well they haven't done it yet. Watching them isn’t enough of a reason to stay for another hour. I need to find a pussy to eat for myself!"
    mct "好吧，他们还没有做。欣赏他俩还不足以成为我再呆一个小时的理由。我得找个妹子自己享用！"

# game/code/d02/s06.rpy:316
translate chinese d02s06_3e11c097:

    # arjt "Oh! [mcname] just left."
    arjt "哦！[mcname]刚走。"

# game/code/d02/s06.rpy:317
translate chinese d02s06_cba77ca9:

    # arjt "But I guess it is good since I will be leaving any moment too."
    arjt "不过我想这是好事，因为我也随时会离开。"

# game/code/d02/s06.rpy:321
translate chinese d02s06_c7ff8a2f:

    # arjt "Oh. Looks like it is time for me to go."
    arjt "哦，看来我该走了。"

# game/code/d02/s06.rpy:325
translate chinese d02s06_4eedf7a5:

    # "[mcname] is on his way to the cafeteria...{w} When suddenly he sees ..."
    "[mcname]正在去食堂的路上……{w}他突然看到……"

# game/code/d02/s06.rpy:330
translate chinese d02s06_354c4361:

    # mct "Damn! There is Lydia!"
    mct "靠！那是莉迪亚！"

# game/code/d02/s06.rpy:331
translate chinese d02s06_9a7faac9:

    # mc "({i}Problem is that she doesn't know that I exist yet. But I'm working on that!{/i})"
    mc "({i} 问题是，她还不知道我的存在。但我正在努力呀！{/i})"

# game/code/d02/s06.rpy:332
translate chinese d02s06_b2d0e7e2:

    # mc "({i}Someone like Pete would just walk up to her and use some cheesy pickup line.{/i})"
    mc "({i}像皮特这样的人就会走到她面前，说一些俗气的话撩她。{/i})"

# game/code/d02/s06.rpy:334
translate chinese d02s06_cdfee642:

    # mct "I'm sure that something like that isn't going to work with her! And I'm not confident enough to try it; I'd probably be super nervous and fuck things up. And with such a fine thing as Lydia I will only get one chance."
    mct "我敢肯定，这一套在她身上是行不通的！不过我也没有信心去尝试，我可能会超级紧张，把事情搞砸。对于莉迪亚这样的尤物，我只有一次机会。"

# game/code/d02/s06.rpy:335
translate chinese d02s06_7122e018:

    # mc "({i}I'm not going to risk it! I need her to know the real me!{/i})"
    mc "({i}我才不会冒这个险的！我要让她了解真实的我！{/i})"

# game/code/d02/s06.rpy:338
translate chinese d02s06_880d2aef:

    # mct "What are the chances that this message on Fetish Locator is actually from Lydia?!"
    mct "性趣探测器上的这条信息是莉迪亚发来的几率到底有多大?!"

# game/code/d02/s06.rpy:339
translate chinese d02s06_121c2921:

    # mc "({i}After all she should be at the private party from the app.{/i})"
    mc "({i}毕竟她是通过软件才能在派对上见到的人。{/i})"

# game/code/d02/s06.rpy:340
translate chinese d02s06_0d03ebf6:

    # mc "({i}Noo! That isn't possible! I can't imagine her letting someone random lick her pussy for points!{/i})"
    mc "({i}不！这不可能啊！我不能想象她会让人随便舔她的小穴来换取点数！{/i})"

# game/code/d02/s06.rpy:341
translate chinese d02s06_fd4ab1b9:

    # mc "({i}But what if it is her? What if she comes there and joins me on that bench?! How should I react?!{/i})"
    mc "({i}但如果真是她呢？如果她过来和我一起坐在长椅上怎么办？我该如何反应?!{/i})"

# game/code/d02/s06.rpy:343
translate chinese d02s06_c701655e:

    # mc "({i}It is probably pointless to stress about that. I should just go and find that fucking banana!{/i})"
    mc "({i}多想这些毫无意义。我应该去找那操蛋的香蕉！{/i})"

translate chinese strings:

    # game/code/d02/s06.rpy:229
    old "Post your lips to find a pussy"
    new "上传嘴唇的照片来吸引小穴"

    # game/code/d02/s06.rpy:229
    old "Better to look for pussy somewhere else"
    new "还是去别的地方找妹子吧"

    # game/code/d02/s06.rpy:267
    old "Sounds shady, better stay away"
    new "听起来很可疑，最好离远点"

    # game/code/d02/s06.rpy:267
    old "Respond to a stranger on the app"
    new "在软件上回复陌生人"

# game/code/d02/s06.rpy:50
translate chinese d02s06_3bcc304f:

    # mct "Did she notice that I came on her yesterday... Would be very awkward if she did..."
    mct "Did she notice that I came on her yesterday... Would be very awkward if she did..."

