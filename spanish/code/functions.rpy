translate spanish strings:

    # game/code/functions.rpy:30
    old "Taboo Edition enabled"
    new "Edición Tabú activada"

    # game/code/functions.rpy:30
    old "Taboo Edition disabled"
    new "Edición Tabú desactivada"

    # game/code/functions.rpy:103
    old "You've unlocked a new CG Gallery"
    new "Has desbloqueado una nueva Galería CG"

    # game/code/functions.rpy:103
    old "You can watch the scene again in the Replay Room"
    new "Puedes ver la escena de nuevo en la Sala de Reproducción"

    # game/code/functions.rpy:103
    old "You've unlocked a new bonus content"
    new "Has desbloqueado un nuevo contenido extra"

    # game/code/functions.rpy:115
    old "Mike"
    new "Mike"

    # game/code/functions.rpy:116
    old "Not_Mike"
    new "Not_Mike"

    # game/code/functions.rpy:128
    old "The game will replay the scene\naccording to a default playthrough"
    new "El juego reproducirá la escena de acuerdo con un desarrollo predeterminado"

    # game/code/functions.rpy:130
    old "The game will try to replay the scene\naccording to your current playthrough"
    new "El juego tratará de reproducir la escena de acuerdo con su actual desarrollo"

