translate spanish strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Lydia"
    new "Día 1: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ignore the girl Pete is sleeping with (name's Hana)"
    new "Ignora a la chica con la que Pete está durmiendo (se llama Hana)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "Día 1: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "Día 2: Mensaje de Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Did you post your lips in the morning?"
    new "¿Pusiste tus labios en la mañana?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Día 2: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "Día 2: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Desbloqueado por no publicar ninguna solicitud en FL o al ignorar la respuesta"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "Día 3: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Debes haber hecho venir a Min el día 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Fiesta: Daisy y Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Elige el evento Santa Oral Secreto"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Fiesta: Chica misteriosa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "Día 4: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "Día 4: La bañera de AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Polly"
    new "Día 4: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "Día 5: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "Día 5: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Debes haber descubierto el secreto de Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "Día 5: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "Día 5: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Dale lecciones privadas a Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Stacy"
    new "Día 6: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "Día 6: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Nora"
    new "Día 6: Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "No hacerle creampie a nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "Día 6: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Debes haber elegido un Santa Oral Secreto pero aún no conoces su secreto"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "Día 6: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "Día 7: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "Día 7: playa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Hana"
    new "Día 7: Hana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "No pidas una liberación"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Daisy"
    new "Día 7: margarita"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Lyssa"
    new "Día 7: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "Día 7: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: AmRose"
    new "Día 7: AmRose"

translate spanish strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "You have seen all the possible pictures that Maria can send"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "Stacy in hell"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Disappoint Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Halloween Bonus - Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Halloween Special"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Halloween Bonus - Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Halloween Bonus - Samiya"

