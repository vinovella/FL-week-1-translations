translate spanish strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Min & Antony"
    new "Día 1: Min y Antony"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to cumshots"
    new "Di sí a las corridas"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Chloe & Peter"
    new "Día 1: Chloe y Peter"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Stacy (flashback)"
    new "Día 2: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Nora & Polly"
    new "Día 2: Nora y Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "You must have posted your lips, met Polly and accepted her conditions"
    new "Debes haber publicado tus labios, conocer a Polly y aceptar sus condiciones"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Min"
    new "Día 2: Min"

    # game/code/data/extended/replay_room.rpy:15
    old "Make Min cum and keep watching her until the end"
    new "Haz que Min se corra y sigue mirándola hasta el final"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - Nora route"
    new "Día 2: Polly - Ruta Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Assfinger Polly"
    new "Dedea el culo de Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - solo"
    new "Día 2: Polly - solo"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Stacy (flashback)"
    new "Día 3: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't scare Stacy"
    new "No asustes a Stacy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Chloe (NTR)"
    new "Día 3: Chloe (NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Save Kevin"
    new "Salva a Kevin"

    # game/code/data/extended/replay_room.rpy:15
    old "Submit to Dahlia"
    new "Someterte a Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Secret Santa"
    new "Fiesta: Santa Secreto"

    # game/code/data/extended/replay_room.rpy:15
    old "Choose the event and request to see Lyssa's pussy"
    new "Elige el evento y solicita ver el coño de Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: QueenB"
    new "Fiesta: QueenB"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle's event"
    new "Fiesta: evento de Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Do whatever it takes to win"
    new "Haz lo que sea necesario para ganar"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Mr.D1cky's event"
    new "Fiesta: evento de Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Allison & Vesna"
    new "Fiesta: Allison y Vesna"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle & Mr.D1cky"
    new "Fiesta: Squizzle y Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Daisy"
    new "Día 4: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Command Daisy to blow you (you must have refused Dahlia's request)"
    new "Ordena a Daisy que te la chupe (debes haber rechazado la solicitud de Dahlia)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: AmRose"
    new "Día 4: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to enema play"
    new "Di sí a los juegos de enema"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Polly & Nora"
    new "Día 4: Polly y Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Finger Polly (any hole)"
    new "Finger Polly (cualquier agujero)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Chloe"
    new "Día 5: Chloe"

    # game/code/data/extended/replay_room.rpy:15
    old "It's showtime! (available if you have supported Kevin's views on NTR)"
    new "¡Es hora del show! (disponible si ha apoyado las opiniones de Kevin en NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Polly"
    new "Día 5: Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Strip and join Polly"
    new "Desnudarse y unirse a polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URCumSlut"
    new "Día 5: TuPutadeCorridas"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Squizzle"
    new "Día 5: Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Earn a gold star"
    new "Gana una estrella dorada"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Daisy"
    new "Día 5: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Invite Daisy and don't be shy"
    new "Invita a Daisy y no seas tímido"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Stacy (flashback)"
    new "Día 5: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Jerk off"
    new "Hacerse una paja"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Daisy"
    new "Día 6: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Daisy"
    new "Creampie a Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Jessie (dorm)"
    new "Día 6: Jessie (dormitorio)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Lydia"
    new "Día 6: Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Nora"
    new "Creampie a Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Polly & Nora"
    new "Día 6: Polly y Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked if you are not romancing Polly"
    new "Desbloqueado si no estás enamorado de Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Take a more dominant role"
    new "Toma un papel más dominante"

    # game/code/data/extended/replay_room.rpy:15
    old "Cum on Vanessa's feet"
    new "Semen en los pies de vanessa"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask for a release"
    new "Solicitar una liberación"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Maria & AmRose (flashback)"
    new "Día 7: Maria y AmRose (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Dahlia"
    new "Día 7: Dalia"

    # game/code/data/extended/replay_room.rpy:15
    old "Spank Lyssa twelve times"
    new "Azota Lyssa doce veces"

    # game/code/data/extended/replay_room.rpy:15
    old "Enjoy the rimjob"
    new "Disfruta del beso negro"

translate spanish strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URLittlePony"
    new "Day 5: URLittlePony"

