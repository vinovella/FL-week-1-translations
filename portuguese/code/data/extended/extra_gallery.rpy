translate portuguese strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Lydia"
    new "Dia 1: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ignore the girl Pete is sleeping with (name's Hana)"
    new "Ignore a garota que Pete está fazendo sexo (o nome é Hana)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "Dia 1: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "Dia 2: Mensagem da Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Did you post your lips in the morning?"
    new "Você postou seus lábios de manhã?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Dia 2: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "Dia 2: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Destravado não postando quaisquer pedido no FL ou ignorando a resposta"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "Dia 3: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Você deve ter feito Min gozar no dia 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Festa Daisy e Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Escolha o evento Amigo Oral secreto"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Festa: Garota misteriosa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "Dia 4: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "Dia 4: Banheira da AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Polly"
    new "Dia 4: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "Dia 5: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "Dia 5: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Você deve tem que ter descoberto o segredo da Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "Dia 5: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "Dia 5: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Dê lições particulares para Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Stacy"
    new "Dia 6: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "Dia 6: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Nora"
    new "Dia 6: Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Não goze dentro da Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "Dia 6: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Você tem que ter escolhido Amigo Oral Secreto, mas ainda não sabe do segredo dela"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "Dia 6: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "Você viu todas as fotos possíveis que Maria pode enviar"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "Dia 7: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "Dia 7: praia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Hana"
    new "Dia 7: Hana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Não peça para gozar"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Daisy"
    new "Dia 7: Daisy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Lyssa"
    new "Dia 7: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "Dia 7: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: AmRose"
    new "Dia 7: AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "Stacy no inferno"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Desaponte Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Bônus de Halloween - Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Especial de Halloween"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Bônus de Halloween - Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Bônus de Halloween - Samiya"

