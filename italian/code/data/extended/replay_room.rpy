translate italian strings:

    # game/code/data/replay_room.rpy:12
    old "Day 1: Min & Antony"
    new "1°: Min e Antony"

    # game/code/data/replay_room.rpy:12
    old "Day 1: Chloe & Peter"
    new "1° giorno: Chloe e Peter"

    # game/code/data/replay_room.rpy:12
    old "Day 1: AmRose"
    new "1° giorno: AmRose"

    # game/code/data/replay_room.rpy:12
    old "Day 2: Stacy (flashback)"
    new "2° giorno: Stacy (flashback)"

    # game/code/data/replay_room.rpy:12
    old "Day 2: Nora & Polly"
    new "2° giorno: Nora e Polly"

    # game/code/data/replay_room.rpy:12
    old "Day 2: Min"
    new "2° giorno: Min"

    # game/code/data/replay_room.rpy:12
    old "Day 2: Polly"
    new "2° giorno: Polly"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Stacy (flashback)"
    new "3° giorno: Stacy (flashback)"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Chloe (NTR)"
    new "3° giorno: Chloe (NTR)"

    # game/code/data/replay_room.rpy:12
    old "Day 3: AmRose"
    new "3° giorno: AmRose"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Dahlia"
    new "3° giorno: Dahlia"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Secret Santa"
    new "3° giorno: Pacco Sorpresa"

    # game/code/data/replay_room.rpy:12
    old "Day 3: QueenB"
    new "3° giorno: QueenB"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Squizzle's event"
    new "3° giorno: evento di Squizzle"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Mr.D1cky's event"
    new "3° giorno: evento di Mr.D1cky"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Allison & Vesna"
    new "3° giorno: Allison e Vesna"

    # game/code/data/replay_room.rpy:12
    old "Day 3: Squizzle & Mr.D1cky"
    new "3° giorno: Squizzle e Mr.D1cky"

    # game/code/data/replay_room.rpy:12
    old "Day 4: Daisy"
    new "4° giorno: Daisy"

    # game/code/data/replay_room.rpy:12
    old "Day 4: AmRose"
    new "4° giorno: AmRose"

    # game/code/data/replay_room.rpy:12
    old "Day 4: Polly"
    new "4° giorno: Polly"

    # game/code/data/replay_room.rpy:12
    old "Day 4: Polly & Nora"
    new "4° giorno: Polly e Nora"

    # game/code/data/replay_room.rpy:12
    old "Day 5: Chloe"
    new "5° giorno: Chloe"

    # game/code/data/replay_room.rpy:12
    old "Day 5: Polly"
    new "5° giorno: Polly"

    # game/code/data/replay_room.rpy:12
    old "Day 5: URLittlePony"
    new "5° giorno: URLittlePony"

    # game/code/data/replay_room.rpy:12
    old "Day 5: Squizzle"
    new "5° giorno: Squizzle"

    # game/code/data/replay_room.rpy:12
    old "Day 5: Daisy"
    new "5° giorno: Daisy"

    # game/code/data/replay_room.rpy:12
    old "Day 5: Stacy (flashback)"
    new "5° giorno: Stacy (flashback)"

    # game/code/data/replay_room.rpy:12
    old "Day 6: Daisy"
    new "6° giorno: Daisy"

    # game/code/data/replay_room.rpy:12
    old "Day 6: Lydia"
    new "6° giorno: Lydia"

    # game/code/data/replay_room.rpy:12
    old "Day 6: Nora"
    new "6° giorno: Nora"

    # game/code/data/replay_room.rpy:12
    old "Day 6: Lyssa"
    new "6° giorno: Lyssa"

    # game/code/data/replay_room.rpy:12
    old "Day 6: Stacy"
    new "6° giorno: Stacy"

    # game/code/data/replay_room.rpy:12
    old "Day 7: Vanessa"
    new "7° giorno: Vanessa"

    # game/code/data/replay_room.rpy:12
    old "Day 7: Hana"
    new "7° giorno: Hana"

    # game/code/data/replay_room.rpy:12
    old "Day 7: Daisy"
    new "7° giorno: Daisy"

    # game/code/data/replay_room.rpy:12
    old "Day 7: Dahlia"
    new "7° giorno: Dahlia"

    # game/code/data/replay_room.rpy:12
    old "Day 7: Lyssa"
    new "7° giorno: Lyssa"

    # game/code/data/replay_room.rpy:12
    old "Day 7: AmRose"
    new "7° giorno: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to cumshots"
    new "Di' sì al fetish del giorno"

    # game/code/data/extended/replay_room.rpy:15
    old "You must have posted your lips, met Polly and accepted her conditions"
    new "Devi avere caricato una foto delle labbra, incontrato Polly e accettato le sue condizioni"

    # game/code/data/extended/replay_room.rpy:15
    old "Make Min cum and keep watching her until the end"
    new "Fa' venire Min e continua a guardarla fino alla fine"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - Nora route"
    new "2° giorno: arco Polly - Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Assfinger Polly"
    new "Sditalina Polly nel culo"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - solo"
    new "2° giorno: Polly (da sola)"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't scare Stacy"
    new "Non spaventare Stacy"

    # game/code/data/extended/replay_room.rpy:15
    old "Save Kevin"
    new "Salva Kevin"

    # game/code/data/extended/replay_room.rpy:15
    old "Submit to Dahlia"
    new "Sottomettiti a Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Secret Santa"
    new "Il party: Pacco Sorpresa"

    # game/code/data/extended/replay_room.rpy:15
    old "Choose the event and request to see Lyssa's pussy"
    new "Scegli l'evento e chiedi a Lyssa di mostrarti la fica"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: QueenB"
    new "Il party: QueenB"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle's event"
    new "Il party: l'evento di Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Do whatever it takes to win"
    new "Fa' di tutto pur di vincere"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Mr.D1cky's event"
    new "Il party: l'evento di Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Allison & Vesna"
    new "Il party: Allison e Vesna"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle & Mr.D1cky"
    new "Il party: Squizzle e Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Command Daisy to blow you (you must have refused Dahlia's request)"
    new "Ordina a Daisy di spompinarti (in precedenza devi avere rifiutato la richiesta di Dahlia)"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to enema play"
    new "Devi avere detto sì ai clisteri"

    # game/code/data/extended/replay_room.rpy:15
    old "Finger Polly (any hole)"
    new "Sditalina Polly (ogni buco è buono, pure quello dell'ozono)"

    # game/code/data/extended/replay_room.rpy:15
    old "It's showtime! (available if you have supported Kevin's views on NTR)"
    new "Che lo spettacolo abbia inizio! (scena accessibile se hai precedentemente appoggiato le vedute di Kevin sul cuckoldismo)"

    # game/code/data/extended/replay_room.rpy:15
    old "Strip and join Polly"
    new "Spogliati e unisciti a Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Earn a gold star"
    new "Ottieni una stella d'oro"

    # game/code/data/extended/replay_room.rpy:15
    old "Invite Daisy and don't be shy"
    new "Invita Daisy e non fare il timido"

    # game/code/data/extended/replay_room.rpy:15
    old "Jerk off"
    new "Segati"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Daisy"
    new "Vieni dentro Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Jessie (dorm)"
    new "6° giorno: Jessie (dormitorio)"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Nora"
    new "Vieni dentro Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Polly & Nora"
    new "6° giorno: Polly e Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked if you are not romancing Polly"
    new "Per sbloccare la scena, NON devi stare nell'arco di Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Take a more dominant role"
    new "Assumi un ruolo dominante"

    # game/code/data/extended/replay_room.rpy:15
    old "Cum on Vanessa's feet"
    new "Vieni sui piedi di Vanessa"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask for a release"
    new "Provaci"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Maria & AmRose (flashback)"
    new "7° giorno: Maria e AmRose (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Spank Lyssa twelve times"
    new "Schiaffeggia Lyssa dodici volte"

    # game/code/data/extended/replay_room.rpy:15
    old "Enjoy the rimjob"
    new "Goditi la leccata"

