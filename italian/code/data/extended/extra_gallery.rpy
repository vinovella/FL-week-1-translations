translate italian strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Lydia"
    new "1° giorno: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ignore the girl Pete is sleeping with (name's Hana)"
    new "Ignora la ragazza che dorme con Pete (si chiama Hana)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "1° giorno: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "2° giorno: messaggio di Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Did you post your lips in the morning?"
    new "La mattina hai spedito un selfie delle tue labbra?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "2° giorno: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Per sbloccare la foto non devi avere postato nessuna richista su FL (o devi avere ignorato la risposta)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "3° giorno: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Il secondo giorno devi avere fatto venire Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Il party: Daisy e Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Scegli il Pacco Sorpresa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Il party: la ragazza misteriosa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "4° giorno: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "4° giorno: il bagno di AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patroni"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "5° giorno: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "5° giorno: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Devi avere scoperto il segreto di Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "5° giorno: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "5° giorno: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Da' lezioni private a Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patroni"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "6° giorno: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Non venire dentro Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "6° giorno: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Al party devi avere scelto il Pacco Sorpresa, ma non devi avere scoperto il suo segreto"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "6° giorno: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "7° giorno: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "7° giorno: la spiaggia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Fa' il bravo e non provarci"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "7° giorno: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "Hai visto tutto le foto che Maria potesse mandarti"

translate italian strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "I dolori della giovane Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Deludi Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Bonus di Halloween - Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Speciale di Halloween"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Bonus di Halloween  - Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Bonus di Halloween - Samiya"

