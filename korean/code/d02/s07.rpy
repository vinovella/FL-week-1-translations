# TODO: Translation updated at 2022-02-08 11:13

# game/code/d02/s07.rpy:99
translate korean d02s07_19452b15:

    # "12:00 PM"
    "오후 12:00"

# game/code/d02/s07.rpy:102
translate korean d02s07_ccc6009a:

    # "[mcname] arrives at the cafeteria. After a short queue he gets a single banana."
    "[mcname]은/는 구내식당에 도착한다. 짧은 줄이 끝나고 그는 바나나 한 개를 구한다. "

# game/code/d02/s07.rpy:103
translate korean d02s07_1b4a430c:

    # "On his way out he notices a familiar face."
    "나가던 길에 익숙한 얼굴을 발견한다."

# game/code/d02/s07.rpy:106
translate korean d02s07_aa5d1abb:

    # mct "So, that's where Pete is!"
    mct "그래, 저기가 피트가 있는 곳이군!"

# game/code/d02/s07.rpy:110
translate korean d02s07_e55888cb:

    # mc "Hey, Pete!"
    mc "야, 피트!"

# game/code/d02/s07.rpy:112
translate korean d02s07_6e272b01:

    # pb "[mcname]! Hey!"
    pb "[mcname]! 어이!"

# game/code/d02/s07.rpy:113
translate korean d02s07_dd58f4f5:

    # mc "Is it your breakfast?"
    mc "그게 니 아침이냐?"

# game/code/d02/s07.rpy:115
translate korean d02s07_02b31895:

    # pb "Hmmm."
    pb "으으음."

# game/code/d02/s07.rpy:116
translate korean d02s07_7859b2e5:

    # pb "I'm not so sure! I still haven't slept since yesterday... {w} It feels more like a late dinner!"
    pb "잘 모르겠는데! 어제 이후로 아직도 잠을 못 잤거든... {w} 늦은 저녁 식사 같은데!"

# game/code/d02/s07.rpy:117
translate korean d02s07_d8f340ff:

    # mc "Wow! That sounds like a hardcore party!?"
    mc "와! 파티가 졸라 빡셌나 보네!??"

# game/code/d02/s07.rpy:119
translate korean d02s07_fc986637:

    # pb "Yes! We went wild with celebration!"
    pb "그래! 이긴 거 때문에 쩔게 했지."

# game/code/d02/s07.rpy:121
translate korean d02s07_c0d0ab33:

    # pb "Come sit with me! I'll tell you how it all went! Or at least the parts that I remember!"
    pb "이리 와서 앉아! 어땠는지 말해줄 게! 내가 아직도 기억하는 한도에서 말야!"

# game/code/d02/s07.rpy:122
translate korean d02s07_ef709518:

    # "[mcname] joins Pete at his table."
    "[mcname]은/는 피트와 함께 테이블에 앉는다."

# game/code/d02/s07.rpy:127
translate korean d02s07_d72a4b71:

    # mc "Nice! I’ve started using the app! Tried to earn some points yesterday."
    mc "그렇지! 나 그 앱 쓰기 시작했다! 어제 점수 좀 벌려고 했지."

# game/code/d02/s07.rpy:129
translate korean d02s07_01ffb007:

    # pb "How did it go?"
    pb "어떻디?"

# game/code/d02/s07.rpy:132
translate korean d02s07_485984f6:

    # mc "Well I earned some with the blitz challenge last night. But not many."
    mc "흠 어제 신속 도전으로 점수 좀 땄어. 하지만 많이는 아냐."

# game/code/d02/s07.rpy:136
translate korean d02s07_5aa55f0d:

    # mc "Wow! That’s a lot of points!"
    mc "와! 점수 많은데!"

# game/code/d02/s07.rpy:138
translate korean d02s07_cce10f86:

    # pb "Yeah, but I guess it was a challenge and at the same time a very kinky thing to do! That is why I got that many points!"
    pb "그렇지, 도전 때문이기도 하고 한 짓이 겁나 변태적이었거든! 그래서 이렇게 점수를 많이 땄지."

# game/code/d02/s07.rpy:139
translate korean d02s07_b806c105:

    # pb "Even if I was actually late to submit. Imagine the points if I had cum in 69 seconds?"
    pb "올리는 게 늦긴 했어도 말야. 69초 안에 쌌으면 땄을 점수가 상상이 되냐?"

# game/code/d02/s07.rpy:141
translate korean d02s07_7a53e763:

    # mc "I guess to get to tomorrow’s party I will have to come up with something like that!"
    mc "내일 파티에 가려면 그런 짓을 떠올려야 하는데!"

# game/code/d02/s07.rpy:148
translate korean d02s07_ac356c9f:

    # mc "I'm working on getting some points today."
    mc "오늘도 점수 좀 벌려고 하고 있어."

# game/code/d02/s07.rpy:151
translate korean d02s07_ef4e5b67:

    # mc "Not really. I was unsuccessful..."
    mc "아냐. 난 실패했어..."

# game/code/d02/s07.rpy:153
translate korean d02s07_a9e13c97:

    # pb "Oh, that’s sad to hear! You need to hurry up! The party is tomorrow night!"
    pb "아, 이거 슬픈 소식구만! 서둘러야 돼! 내일 저녁에 파티라고!"

# game/code/d02/s07.rpy:154
translate korean d02s07_987394fa:

    # pb "There was an additional challenge last night. I got a lot of points from it!"
    pb "지난 밤에 추가 도전이 있었잖아. 그걸로 점수를 꽤 땄다니까!"

# game/code/d02/s07.rpy:156
translate korean d02s07_916f4145:

    # mc "I guess to get to the party I would have to come up with something epic."
    mc "내일 파티에 가려면 전설적인 걸 떠올려야 하는데."

# game/code/d02/s07.rpy:158
translate korean d02s07_6b653a62:

    # pb "You can just do several small things. You don't have to win a challenge to get all the points you need!"
    pb "그냥 작은 일을 여러번 할 수도 있어. 점수를 한 번에 다 따려고 도전에서 이길 필요는 없다고!"

# game/code/d02/s07.rpy:160
translate korean d02s07_803742a9:

    # mc "Okay, I will try to come up with something!"
    mc "좋아, 뭔가 생각해볼께!"

# game/code/d02/s07.rpy:161
translate korean d02s07_17fdb1f4:

    # mc "I’m gonna get going! See you!"
    mc "난 가야겠다! 또 봐!"

# game/code/d02/s07.rpy:164
translate korean d02s07_7a11f59f:

    # pb "Ciao!"
    pb "차우!"

# game/code/d02/s07.rpy:167
translate korean d02s07_0f18c60c:

    # pb "And good luck with whatever you plan to do with this banana!"
    pb "그리고 그 바나나로 뭘 하든 간에 잘 하길 빈다!"

# game/code/d02/s07.rpy:168
translate korean d02s07_ab3e2ca2:

    # mc "Hahaha! Funny one!"
    mc "하하하! 재미있는 녀석!"

# game/code/d02/s07.rpy:169
translate korean d02s07_57f63532:

    # mct "I hope that I actually don't have to do anything with it!"
    mct "정말 이걸로 뭘 할 필요는 없겠지!"

# game/code/d02/s07.rpy:171
translate korean d02s07_1f9c1d0e:

    # mc "({i}It just occurred to me ... when my [game.stacytitle] comes here she will definitely find out about Fetish Locator, since everyone is using it here. Will she use it too?!{/i})"
    mc "({i}갑자기 생각났는데... 내 [game.stacytitle]이/가 여기에 오면 분명히 페티시 로케이터가 뭔지 알게 될거란 말야, 여기서는 모두가 이 앱을 쓰니까. 걔도 이걸 쓸까?!{/i})"

# game/code/d02/s07.rpy:181
translate korean replay_d02s07_b50077a0:

    # mc "({i}Sleeping on this couch is horrible! I am already dreaming about going back to my dorm bed!{/i})"
    mc "({i}침대에서 자는 건 끔찍했어! 벌써 기숙사 침대로 그리워지네!{/i})"

# game/code/d02/s07.rpy:182
translate korean replay_d02s07_11cf90e9:

    # mc "({i}Ugh, I need a shower!{/i})"
    mc "({i}윽, 샤워가 필요해!{/i})"

# game/code/d02/s07.rpy:184
translate korean replay_d02s07_c3b6099b:

    # mct "Is everyone still sleeping?"
    mct "아직도 다들 자는 건가?"

# game/code/d02/s07.rpy:186
translate korean replay_d02s07_eca82bf1:

    # mct "Great! The door is open! The shower shall be mine!"
    mct "훌륭해! 문은 열려 있군. 샤워는 나의 것이 되리라!"

# game/code/d02/s07.rpy:192
translate korean replay_d02s07_b5fc3d66:

    # mct "Strange... I hear the shower running."
    mct "이상한 걸... 샤워 소리가 들리잖아."

# game/code/d02/s07.rpy:200
translate korean replay_d02s07_f0042343:

    # mct "What? Again?!"
    mct "뭐야? 또?!"

# game/code/d02/s07.rpy:202
translate korean replay_d02s07_63f812be:

    # mc "({i}Why on earth isn’t the door closed?!{/i})"
    mc "({i}누가 세상천지에 문을 열어 놓고 샤워를 하는 거야?!{/i})"

# game/code/d02/s07.rpy:203
translate korean replay_d02s07_4355860f:

    # mc "({i}Again I walk in on my [game.stacytitle], but this time she’s actually naked!{/i})"
    mc "({i}또 내 [game.stacytitle]이/가 있는 방에 들어왔네, 게다가 이번에는 정말로 벗고 있잖아!{/i})"

# game/code/d02/s07.rpy:204
translate korean replay_d02s07_cf1cd28e:

    # mc "({i}What should I do?!{/i})"
    mc "({i}어떻게 해야 하지?!{/i})"

# game/code/d02/s07.rpy:213
translate korean replay_d02s07_db0389aa:

    # mc "({i}She is my [game.stacytitle]! I should leave immediately!{/i})"
    mc "({i}얘는 내 [game.stacytitle]이야! 지금 나가야 돼!{/i})"

# game/code/d02/s07.rpy:216
translate korean replay_d02s07_f9ea10f1:

    # mct "I will just open the door a little bit more!"
    mct "그냥 문을 조금만 더 열 뿐이야!!"

# game/code/d02/s07.rpy:220
translate korean replay_d02s07_87cb12c3:

    # mct "Wow! I see Stacy naked!"
    mct "와! 내가 스테이시가 벗은 걸 보다니!"

# game/code/d02/s07.rpy:221
translate korean replay_d02s07_a2c85cd6:

    # mc "({i}I will watch just a little bit more!{/i})"
    mc "({i}그냥 조금만 더 보는 거야!{/i})"

# game/code/d02/s07.rpy:241
translate korean replay_d02s07_a18006d4:

    # mct "And now she’s turned off the shower! I should leave now!"
    mct "이제 물을 잠갔잖아! 이제 떠나야 해!"

# game/code/d02/s07.rpy:242
translate korean replay_d02s07_66f66cf3:

    # mct "Oh! Is she about to do what I think?"
    mct "오! 얘가 설마 그걸 하려는 건가?"

# game/code/d02/s07.rpy:243
translate korean replay_d02s07_af60218e:

    # mct "I pee in the shower too. But should I watch how she does it?"
    mct "나도 샤워 중에 오줌을 싸긴 하지. 그래도 얘가 하는 걸 봐야 할까?"

# game/code/d02/s07.rpy:248
translate korean replay_d02s07_a7b96636:

    # mct "Will she actually pee?"
    mct "진짜 오줌을 싸려는 거야?"

# game/code/d02/s07.rpy:249
translate korean replay_d02s07_a8656b26:

    # mct "Why do I want to watch my [game.stacytitle] piss in the shower?"
    mct "내 [game.stacytitle]이/가 샤워 중에 오줌 싸는 걸 왜 보고 싶은 거지?"

# game/code/d02/s07.rpy:250
translate korean replay_d02s07_e54639ee:

    # mct "Why am I so aroused by that thought?"
    mct "그 생각만 했는데 내가 왜 이렇게 흥분을 했지?"

# game/code/d02/s07.rpy:256
translate korean replay_d02s07_22fa5f40:

    # mct "She is actually pissing now!"
    mct "지금 정말로 오줌을 싸잖아!"

# game/code/d02/s07.rpy:257
translate korean replay_d02s07_a13dfa49:

    # mct "I'm watching my naked [game.stacytitle] pee!"
    mct "내 [game.stacytitle]이/가 알몸으로 오줌 싸는 걸 보고 있다니!"

# game/code/d02/s07.rpy:270
translate korean replay_d02s07_6d530174:

    # mct "Wow! Wait! What is she doing?!"
    mct "와! 잠깐! 얘가 뭘 하고 있는 거야?!"

# game/code/d02/s07.rpy:271
translate korean replay_d02s07_04cfb5d9:

    # mct "Is she masturbating while peeing? {w} Or she is peeing while masturbating!?"
    mct "오줌을 싸면서 자위를 한다고? {w} 아님 자위를 하면서 오줌을 싸는 건가!?"

# game/code/d02/s07.rpy:276
translate korean replay_d02s07_b0a5fdcf:

    # sy "Mmmh"
    sy "음으으흥"

# game/code/d02/s07.rpy:280
translate korean replay_d02s07_21301221:

    # sy "Yeah"
    sy "아"

# game/code/d02/s07.rpy:282
translate korean replay_d02s07_f8eec004:

    # mc "({i}Now if I get caught, I'm not only spying on [game.stacytitle] in shower, but I'm spying on [game.stacytitle] pissing and masturbating!{/i})"
    mc "({i}이제 들키게 되면, 내 [game.stacytitle]이/가 샤워 하는 걸 훔쳐봤을 뿐만 아니라, 내 [game.stacytitle]이/가 오줌 싸면서 자위하는 걸 훔쳐본 게 된다고!{/i})"

# game/code/d02/s07.rpy:284
translate korean replay_d02s07_9c9f6619:

    # mc "({i}And last time I spied on her I got caught!{/i})"
    mc "({i}그러고 전에 훔쳐봤을 때도 결렸잖아!{/i})"

# game/code/d02/s07.rpy:285
translate korean replay_d02s07_e99614ac:

    # mc "({i}I’ve got an erection from my [game.stacytitle]. That is so wrong and at the same time so incredibly hot!{/i})"
    mc "({i}내 [game.stacytitle] 때문에 발기를 했어. 이건 정말 잘못된 일이지만 동시에 진짜 엄청나게 꼴려!{/i})"

# game/code/d02/s07.rpy:289
translate korean replay_d02s07_72bdd4f1:

    # "After Stacy`s piss stream is running to an end, she collapses on her ass, splashing drops of pee all over the place."
    "스테이시는 오줌 줄기가 끝나자, 엉덩이로 그 자리에 주저 앉는다. 그러자 오줌 방울이 사방으로 튄다."

# game/code/d02/s07.rpy:290
translate korean replay_d02s07_859c321f:

    # mct "Looks like she is going to need another shower soon."
    mct "금방 다시 샤워를 해야 하겠는 걸."

# game/code/d02/s07.rpy:292
translate korean replay_d02s07_f71e9b49:

    # "She grabs left nipple with her right hand and starts to pull it forward with every movement as her left hand is going down."
    "그녀는 오른손으로 왼쪽 젖꼭지를 잡고 왼손이 아래로 내려갈 때마다 젖꼭지를 앞으로 잡아당기기 시작한다."

# game/code/d02/s07.rpy:299
translate korean replay_d02s07_1a8deaac:

    # "Stacy starts to collapse backwards."
    "스테이스는 뒤로 쓰러지기 시작한다."

# game/code/d02/s07.rpy:302
translate korean replay_d02s07_919f3d91:

    # "When her bare ass touched the wet floor a scream of pleasure came out of her mouth."
    "엉덩이가 젖은 바닥에 닿자 그녀의 입에서 기쁨에 찬 탄성이 흘러나온다."

# game/code/d02/s07.rpy:303
translate korean replay_d02s07_9731f864:

    # sy "A-a-a-h"
    sy "아-아-아-앗"

# game/code/d02/s07.rpy:307
translate korean replay_d02s07_ae0e3462:

    # "Stacy’s body starts to shake in orgasmic spasms!"
    "스테이시의 몸은 절정의 경련으로 떨리기 시작한다!"

# game/code/d02/s07.rpy:315
translate korean replay_d02s07_5c25b3ee:

    # sy "O-o-o-h"
    sy "오-오-오-옷"

# game/code/d02/s07.rpy:316
translate korean replay_d02s07_2d8f6d5d:

    # "After bursting in orgasm her muscles relaxed and she continued falling onto her back"
    "절정이 터지자 그녀의 근육은 이완되고 그녀는 뒤로 무너진다"

# game/code/d02/s07.rpy:337
translate korean replay_d02s07_d7f7bcb2:

    # mct "I... {w}think I... {w}came in my pants... {w} That was amazing!"
    mct "내... {w}생각엔... 내가 {w}바지에 싼 것 같아... {w} 존나 쩐다!"

# game/code/d02/s07.rpy:353
translate korean replay_d02s07_52a9ffc5:

    # mc "({i}My [game.stacytitle] is a kinky one. From what I know of her, she won’t hesitate to use the app!{/i})"
    mc "({i}내 [game.stacytitle]이/가 이렇게 변태라니. 내가 생각엔, 얘는 주저 없이 그 앱을 쓸 거야!{/i})"

# game/code/d02/s07.rpy:354
translate korean replay_d02s07_6b2c50de:

    # mc "({i}But I have this feeling of being protective towards her. I don't want her to post sexy pictures for other dudes to see...{/i})"
    mc "({i}하지만 내가 보호해 줘야 돼. 얘가 다른 놈이 보라고 섹시한 사진을 올리게 할 수 없어...{/i})"

# game/code/d02/s07.rpy:355
translate korean replay_d02s07_b7389156:

    # mc "({i}Or am I actually just jealous?!{/i})"
    mc "({i}혹시 지금 내가 정말 질투를 하는 건가?!{/i})"

# game/code/d02/s07.rpy:357
translate korean replay_d02s07_c2ae07eb:

    # mc "({i}Time to find out what the deal is with this banana.{/i})"
    mc "({i}이제 바나나로 뭘 하는지 알아볼 시간이군.{/i})"

# game/code/d02/s07.rpy:371
translate korean d01s07_pete_about_girls_fcc1b776:

    # pb "Hmmm, what was I talking about?"
    pb "으으음, 내가 무슨 말을 하고 있었지?"

# game/code/d02/s07.rpy:372
translate korean d01s07_pete_about_girls_16af2776:

    # mc "The party."
    mc "그 파티."

# game/code/d02/s07.rpy:377
translate korean d01s07_pete_about_girls_2854182f:

    # pb "Oh, yeah! There were some crazy girls there! They were not from our college so they never heard of the Fetish Locator app! We had to show it to them!"
    pb "아, 그래! 그 파티엔 정말 말도 안되는 여자들도 좀 올 거야! 걔들은 우리 대학 애들이 아니니까 페티시 로케이터 앱에 관해 들어 본 적도 없을 걸! 우리가 이걸 걔들한테 보여 줘야 된다고!"

# game/code/d02/s07.rpy:388
translate korean d01s07_pete_about_girls_2_cd0e4703:

    # pb "Right! I remember that blitz challenge! I was fucking this sporty blond girl in her super tight ass and then noticed the app notification."
    pb "맞아! 그 신속 도전 기억난다! 내가 그 발랄한 금발 년의 졸라게 조이는 엉덩이에 박고 있었는데 앱 알림이 오더라고. "

# game/code/d02/s07.rpy:389
translate korean d01s07_pete_about_girls_2_4240b1bf:

    # pb "I think I opened the app a bit late, but I still pulled my dick out of her ass, stood up right above her and came all over the top of her head!"
    pb "내 기억으론 앱을 좀 있다가 열였지, 그래도 바로 그년 엉덩이에서 좆을 뺀 다음, 걔 바로 위에 섰어 그다음에 머리 전체에다가 쌌지."

# game/code/d02/s07.rpy:391
translate korean d01s07_pete_about_girls_2_0741f128:

    # pb "Right on her pretty blond hair! Can you believe that?! It was epic!"
    pb "바로 그년의 그 예쁜 금발 머리 위에 샀다니까! 믿겨지냐?! 이거 정말 졸라 엄청 쩔었어!"

# game/code/d02/s07.rpy:393
translate korean d01s07_pete_about_girls_2_5b5eb208:

    # pb "Got 25 points for that!"
    pb "그걸로 25점을 땄다니까!"

translate korean strings:

    # game/code/d02/s07.rpy:205
    old "Keep watching"
    new "계속 본다"

    # game/code/d02/s07.rpy:205
    old "Stop spying on your [game.stacytitle]"
    new "당신의 [game.stacytitle]을/를 훔쳐보는 걸 그만 둔다"

    # game/code/d02/s07.rpy:245
    old "Continue watching [game.stacytitle]"
    new "계속 [game.stacytitle]을/를 본다"

    # game/code/d02/s07.rpy:245
    old "Let’s have some boundaries and leave"
    new "선을 넘지 않고 떠난다"
