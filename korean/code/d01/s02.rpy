# TODO: Translation updated at 2022-02-08 11:13

# game/code/d01/s02.rpy:86
translate korean d01s02_4ee8cc8f:

    # "9:00 AM"
    "오전 9:00"

# game/code/d01/s02.rpy:88
translate korean d01s02_5b6f8a2b:

    # mct "Back to my normal routine and another history class."
    mct "평범한 일상으로 돌아가서 역사 수업을 들어야지."

# game/code/d01/s02.rpy:90
translate korean d01s02_0fc96c86:

    # mct "Antony and Min sitting together?!"
    mct "앤토니와 민이 함께 앉아있네?!"

# game/code/d01/s02.rpy:92
translate korean d01s02_ccff9ce5:

    # mct "They broke up over the winter break..."
    mct "얘들은 겨울 방학에 헤어졌는데..."

# game/code/d01/s02.rpy:94
translate korean d01s02_ad0fa766:

    # mct "A couple weeks ago they still hated each other"
    mct "2주 전만 해도 서로를 싫어하더니"

# game/code/d01/s02.rpy:96
translate korean d01s02_f3e611a0:

    # mct "Look at them now. Are they dating again?"
    mct "지금은 봐봐. 다시 사귄다고?"

# game/code/d01/s02.rpy:98
translate korean d01s02_6dd4c992:

    # mes "What day is it?"
    mes "언제 파티를 하기로 했지?"

# game/code/d01/s02.rpy:100
translate korean d01s02_0b3fb849:

    # am "Wednesday."
    am "수요일."

# game/code/d01/s02.rpy:102
translate korean d01s02_ccd1b5eb:

    # mes "Who'll be there?"
    mes "누가 오는데?"

# game/code/d01/s02.rpy:104
translate korean d01s02_b1b76aad:

    # am "Just a small group of friends. Do you want Lydia to come?"
    am "그냥 친구 명 몇 정도. 리디아도 부를까?"

# game/code/d01/s02.rpy:106
translate korean d01s02_ba17f776:

    # mes "We have to invite her."
    mes "당연히 걜 초대해야지."

# game/code/d01/s02.rpy:108
translate korean d01s02_3482a888:

    # am "...inaudible..."
    am "...알아들을 수 없는 소리..."

# game/code/d01/s02.rpy:110
translate korean d01s02_7ebd2479:

    # mct "Hmm... interesting... Looks like a lot has happened since I was here last."
    mct "흐음... 흥미로운 걸... 내가 없던 사이에 많은 일이 있었던 모양이네."

# game/code/d01/s02.rpy:112
translate korean d01s02_331f710e:

    # mct "And they want to invite Lydia. That is even more interesting."
    mct "그리고 쟤들은 리디아를 초대하려고 한단 말이지. 더 흥미가 돋는 걸."

# game/code/d01/s02.rpy:115
translate korean d01s02_a20cefa7:

    # "..."
    "..."

# game/code/d01/s02.rpy:117
translate korean d01s02_aba68006:

    # mc "Hey Kevin!"
    mc "야, 케빈!"

# game/code/d01/s02.rpy:119
translate korean d01s02_f0bf8bf4:

    # kb "Hey! Long time, no see. Welcome back. How's it goin' today?"
    kb "이야! 오랜만에 본다. 잘 돌아왔어. 오늘은 좀 어때?"

# game/code/d01/s02.rpy:121
translate korean d01s02_f9a2df6c:

    # mc "Been better."
    mc "더할 나위 없지 좋지."

# game/code/d01/s02.rpy:123
translate korean d01s02_74e404bd:

    # mc "Managed to get a detention today."
    mc "오늘 학교에 남아서 작업을 해야 돼."

# game/code/d01/s02.rpy:124
translate korean d01s02_bb801ea7:

    # mc "I will be cleaning after the basketball game."
    mc "농구 경기가 끝나면 청소를 해야 한다고."

# game/code/d01/s02.rpy:126
translate korean d01s02_9fc29083:

    # kb "Miss Waller?"
    kb "월러 사감이 시킨 거야?"

# game/code/d01/s02.rpy:128
translate korean d01s02_df7f0366:

    # mc "Yea, she saw a girl leaving our room that Pete had over last night."
    mc "그래, 사감이 피트랑 잤던 여자애가 우리 방에서 나가는 걸 봤거든."

# game/code/d01/s02.rpy:130
translate korean d01s02_3a75ee50:

    # kb "Ouch. That sucks!"
    kb "이야, 엿 같겠네!"

# game/code/d01/s02.rpy:132
translate korean d01s02_492177d9:

    # mc "I woke up this morning to see a hot girl in front of me...right across the room! Can you imagine?!"
    mc "아침에 일어나니까 새끈한 애가 내 앞에 보이더라...바로 건너편에 말야! 상상이 돼냐?!"

# game/code/d01/s02.rpy:134
translate korean d01s02_28e2321d:

    # kb "That doesn't sound so bad."
    kb "그건 그렇게 나쁘지 들리지 않는데."

# game/code/d01/s02.rpy:136
translate korean d01s02_55fb62ef:

    # mc "If only she was my girl. Pete brought her to our room...and you know how risky that is with our resident advisor!"
    mc "정말 걔가 내 여자였으면 좋았을텐데. 피트가 걜 우리 방에 데려왔어. 우리 사감이 얼마나 까다로운지 알잖아!"

# game/code/d01/s02.rpy:138
translate korean d01s02_b11c099d:

    # kb "True story! You can't joke around with Miss Waller."
    kb "그건 레알이지! 누가 월러 사감이랑 장난을 하겠어."

# game/code/d01/s02.rpy:140
translate korean d01s02_bc5dc4cc:

    # mc "Seems the lecture is starting."
    mc "강의를 시작하려는 모양이다."

# game/code/d01/s02.rpy:144
translate korean d01s02_12655056:

    # rn "Quiet, please! Today, we'll be talking about..."
    rn "조용히! 오늘 우리가 이야기할 것은..."

# game/code/d01/s02.rpy:145
translate korean d01s02_d59a0b93:

    # rn "Bla-blah-blah"
    rn "이러쿵저러쿵"

# game/code/d01/s02.rpy:146
translate korean d01s02_314849a5:

    # mct "Oh, damn! I already forgot how boring this is."
    mct "아, 젠장! 이 강의가 얼마나 지루한지 벌써 까먹고 있었다."

# game/code/d01/s02.rpy:148
translate korean d01s02_0bd30138:

    # kb "How was your trip home? I forgot, why did you go?"
    kb "본가로 갔던 건 어땠어? 벌써 까먹었네, 니가 왜 갔더라?"

# game/code/d01/s02.rpy:150
translate korean d01s02_4ab34b9f:

    # mc "My [game.stacytitle] is coming here to study at our college. I went home to help her pack, then got sick, and had to stay there until I felt well enough to travel again."
    mc "내 [game.stacytitle]이/가 우리 대학에 입학해. 짐 싸는 걸 도와주러 집에 갔는데, 몸이 안 좋아졌어. 그래서 다시 돌아올 수 있을 때까지 쉬어야 했지."

# game/code/d01/s02.rpy:152
translate korean d01s02_b2a40943:

    # kb "Oh, wow!"
    kb "와, 이런!"

# game/code/d01/s02.rpy:154
translate korean d01s02_8e9364a8:

    # mc "What did I miss?"
    mc "내가 없는 사이에 뭐 재미있는 것 좀 있었어?"

# game/code/d01/s02.rpy:156
translate korean d01s02_aaef13fd:

    # kb "There was some new stuff I guess. Everyone is stressed about the finals."
    kb "뭐 좀 새로운 게 있긴 했어. 기말고사 때문에 모두가 스트레스를 받고 있거든."

# game/code/d01/s02.rpy:158
translate korean d01s02_a3f1d4fd:

    # kb "Oh, right, and you might have noticed that Antony and Min are back together."
    kb "아, 그렇지. 앤토니랑 민이 다시 합친 건 봤지?"

# game/code/d01/s02.rpy:160
translate korean d01s02_94ba6d1d:

    # mc "Yeah, I heard them talking about some kind of party."
    mc "그래, 내가 듣기론 무슨 파티 같은 걸 이야기하더라."

# game/code/d01/s02.rpy:162
translate korean d01s02_d8095c2a:

    # kb "I don't know about any party."
    kb "파티 같은 건 잘 모르겠는데."

# game/code/d01/s02.rpy:163
translate korean d01s02_4bc28f05:

    # kb "Hmm...maybe they want to throw a party to celebrate the fact they're back together?"
    kb "흠... 다시 합쳤다고 파티를 열려고 하는 건가?"

# game/code/d01/s02.rpy:165
translate korean d01s02_0999e95f:

    # mc "Unless they break up again before the party!"
    mc "파티 전에 다시 헤어지지 않으면 열겠지!"

# game/code/d01/s02.rpy:167
translate korean d01s02_b6a99c29:

    # kb "Ahahahah! Right! Or they might break up at the party!"
    kb "아하하하! 맞아! 아니면 파티에서 갈라설 수도 있지!"

# game/code/d01/s02.rpy:168
translate korean d01s02_e473e3b9:

    # mc "Ahahaahahah!"
    mc "아하하아하하!"

# game/code/d01/s02.rpy:170
translate korean d01s02_849955f3:

    # kb "Anyway, are you ready for finals?"
    kb "그건 그렇고, 기말고사 준비는 했냐?"

# game/code/d01/s02.rpy:172
translate korean d01s02_3d70f645:

    # mc "I tried to study last night, but I wasn't even able to get caught up with the stuff from the last two weeks that I missed."
    mc "어젯밤에 공부하려고 하긴 했는데, 2주 동안 놓친 걸 따라잡지 못하겠더라."

# game/code/d01/s02.rpy:173
translate korean d01s02_e3497235:

    # mc "I was so tired I could have slept through an earthquake {i}...and probably did{/i} after that. I didn't even hear Pete fucking that chick!"
    mc "공부 좀 하니까 지진이 나도 잠에서 안 깰 정도로 피곤했어... {i}분명히 그랬던 거야{/i}. 피트가 그 여자랑 떡치는 걸 알지도 못했다니까"

# game/code/d01/s02.rpy:175
translate korean d01s02_680a44d1:

    # kb "At least you tried."
    kb "적어도 공부를 하려고는 했잖아."

# game/code/d01/s02.rpy:177
translate korean d01s02_0edae168:

    # mc "I don't think it'll make a difference."
    mc "그렇다고 뭐가 달라지겠냐."

# game/code/d01/s02.rpy:178
translate korean d01s02_57555de8:

    # mc "How was Sunday?"
    mc "일요일에는 뭐했냐?"

# game/code/d01/s02.rpy:180
translate korean d01s02_26e102ad:

    # kb "We went out with Chloe...had some fun."
    kb "클로이랑 같이 놀았어... 그리고 재미 좀 봤지."

# game/code/d01/s02.rpy:182
translate korean d01s02_34948fd5:

    # mc "Nice! Your [game.chloekevin] is doing fine?"
    mc "잘했네! 니네 [game.chloekevin]은/는 잘 지내지?"

# game/code/d01/s02.rpy:184
translate korean d01s02_ed823cb2:

    # kb "Yeah, she is doing great!"
    kb "그럼, 걔는 엄청 잘 지내지!"

# game/code/d01/s02.rpy:186
translate korean d01s02_91e0cd29:

    # mct "Let's try to listen to the lecture."
    mct "이제 강의 좀 듣자."

# game/code/d01/s02.rpy:187
translate korean d01s02_7e43f6bc:

    # "About ten minutes later."
    "약 10분 후."

# game/code/d01/s02.rpy:190
translate korean d01s02_ef432d66:

    # mct "That's something new."
    mct "저건 새로운 거네."

# game/code/d01/s02.rpy:191
translate korean d01s02_1e70073e:

    # mc "Never seen that app. What is this?"
    mc "그 앱 전에 못 보던 건데. 그거 뭐냐?"

# game/code/d01/s02.rpy:193
translate korean d01s02_b49f37d6:

    # kb "Yeah. It's new. I found it a of couple weeks ago. Somebody local made it."
    kb "맞아. 이거 새 앱이야. 몇 주 전에 찾았는데. 근처 사람이 만들었어."

# game/code/d01/s02.rpy:195
translate korean d01s02_eb9b3580:

    # mc "What does it do?"
    mc "뭐 하는 앱인데?"

# game/code/d01/s02.rpy:197
translate korean d01s02_88853ef5:

    # kb "It's called Fetish Locator. It's like a dating app, but looks for people with the same sexual fetishes you have."
    kb "페티시 로케이터라는 앱이야. 데이트 앱 같은 건데, 니가 바라는 성적 페티시를 갖진 사람을 찾는 거지."

# game/code/d01/s02.rpy:199
translate korean d01s02_770c683b:

    # mc "Hmm...aren't \"looking for sex\" apps mostly dudes? And all the girls fake?"
    mc "으음... \"섹스 물색용\" 앱은 주로 남자놈들이 쓰는 거 아니냐? 그리고 그 앱에 나오는 여자애들은 다 가짜잖아?"

# game/code/d01/s02.rpy:201
translate korean d01s02_e8a9d7ec:

    # kb "Well, it isn't just sex. Sexual compatibility with the same fetishes could be a good start for a strong relationship."
    kb "에... 이건 그냥 섹스 앱이 아냐. 같은 페티시를 통한 성적 융합성은 굳건한 관계를 위한 좋은 시작이 될 수 있다고."

# game/code/d01/s02.rpy:202
translate korean d01s02_21b8afaf:

    # kb "Anyway, it's just in beta right now, but there are a lot students on campus already using it."
    kb "아무튼, 지금 베타 버전인데도, 캠퍼스에서 많은 학생들이 이 앱을 쓰고 있어."

# game/code/d01/s02.rpy:203
translate korean d01s02_3b056b7a:

    # kb "I don't think anyone outside of our college knows about it."
    kb "우리 대학 밖 사람들은 모르는 모양이야."

# game/code/d01/s02.rpy:204
translate korean d01s02_8e1260f5:

    # kb "Your info is kept anonymous and it's only shown to someone if you want it to be. You can control the settings in the app."
    kb "니 정보는 익명으로 저장되고 오직 니가 원하는 경우에만 다른 사람한테 노출돼. 앱 설정에서 이 설정을 바꿀 수 있어."

# game/code/d01/s02.rpy:206
translate korean d01s02_a106da0d:

    # mc "You sound like a salesman! Do you work for them?"
    mc "무슨 장사꾼처럼 말하냐! 그 회사에서 일하냐?"

# game/code/d01/s02.rpy:208
translate korean d01s02_cc12458d:

    # kb "..."
    kb "..."

# game/code/d01/s02.rpy:209
translate korean d01s02_bb14e7db:

    # mc "I'm kidding! Chill! How do I join?"
    mc "장난이야! 진정해! 어떻게 가입하냐?"

# game/code/d01/s02.rpy:211
translate korean d01s02_3ec570d3:

    # kb "Here's the code. You can use it to download the app."
    kb "여기 주소. 이걸로 앱을 다운로드 할 수 있어."

# game/code/d01/s02.rpy:213
translate korean d01s02_9dfad29e:

    # mct "What if Lydia downloaded the app too?! I have to try it."
    mct "리디아도 이 앱을 받았으면 어쩌지?! 반드시 써봐야 돼."

# game/code/d01/s02.rpy:214
translate korean d01s02_a0a0eb0c:

    # mc "Thanks. I'll try it out."
    mc "고마워. 한번 써 볼게."

# game/code/d01/s02.rpy:215
translate korean d01s02_28cd5125:

    # mct "Let's download it and see."
    mct "다운로드 받아보자."

# game/code/d01/s02.rpy:218
translate korean d01s02_7a6a42e2:

    # mct "Wow! Looks nice."
    mct "와! 괜찮아 보이는 걸."

# game/code/d01/s02.rpy:220
translate korean d01s02_894e1070:

    # mct "It wants me to register to use it."
    mct "쓰려면 등록하라는 거네."

# game/code/d01/s02.rpy:221
translate korean d01s02_8c8fbdfc:

    # mct "Okay, I'm supposed to enter my name. I'm not stupid enough to put my real name {i}and{/i} list of my fetishes! I'll make up a fake name."
    mct "좋았어, 이름을 입력해야 하나 보네. 진짜 이름 {i}하고{\i} 내 페티시 목록을 입력할 정도로 난 멍청하진 않단 말이지! 가짜 이름을 쓸 거야."

# game/code/d01/s02.rpy:231
translate korean d01s02_0b7e083d:

    # mct "Done. But nothing is happening..."
    mct "완료. 그런데 아무 일도 안 일어나잖아..."

# game/code/d01/s02.rpy:233
translate korean d01s02_f5ea78a4:

    # mct "Let's try to listen to the professor."
    mct "교수님 이야기를 들을려고나 해보자."

# game/code/d01/s02.rpy:234
translate korean d01s02_0893f58e:

    # "Around ten minutes later..."
    "약 10분 후..."

# game/code/d01/s02.rpy:237
translate korean d01s02_c23c0e83:

    # mct "Oh, there's a push notification from the app with a question."
    mct "아, 앱에서 질문 형식으로 푸시 알림이 뜨는구나."

# game/code/d01/s02.rpy:241
translate korean d01s02_fcbe40cd:

    # fl "Do you like Cumshot fetish?"
    fl "컴샷(Cumshot) 페티시를 좋아하십니까?"

# game/code/d01/s02.rpy:242
translate korean d01s02_015d20de:

    # mct "Wow!"
    mct "세상에!"

# game/code/d01/s02.rpy:244
translate korean d01s02_699878f6:

    # mc "Hey!"
    mc "야!"

# game/code/d01/s02.rpy:246
translate korean d01s02_2c28bb2f:

    # mc "Why did it only send me one question just about one fetish?"
    mc "이 앱이 왜 페티시 하나에 관한 질문만 보낸 거냐?"

# game/code/d01/s02.rpy:247
translate korean d01s02_f6203809:

    # kb "About Cumshot?"
    kb "컴샷에 관한 거?"

# game/code/d01/s02.rpy:249
translate korean d01s02_786856f5:

    # mc "Did we all get the same question?"
    mc "우리 모두 같은 질문을 받은 거야?"

# game/code/d01/s02.rpy:251
translate korean d01s02_5ca54ecf:

    # kb "Yeah. the app has, like, a fetish of the day. And everyone is asked about it."
    kb "그렇지. 이 앱은, 그러니까, 오늘의 페티시 같은 게 있어. 그리고 모두가 오늘의 페티시에 관한 질문을 받지."

# game/code/d01/s02.rpy:253
translate korean d01s02_5f431293:

    # mc "Hmmm... I see. I thought that I'd be able to enter all the things I like."
    mc "으으음... 알겠다. 난 내가 좋아하는 건 다 들어갈 수 있을 거라고 생각했는데."

# game/code/d01/s02.rpy:255
translate korean d01s02_cdd73384:

    # kb "Yeah, it works a little differently and the app is always changing. Sometimes, it does strange things. You'll see."
    kb "그래, 약간 다르게 작동하고 앱이 계속 변해. 가끔은 이상한 짓도 한다니까. 곧 보게 될 거야."

# game/code/d01/s02.rpy:257
translate korean d01s02_4076254e:

    # mc "Okay, thanks."
    mc "알았어, 고마워."

# game/code/d01/s02.rpy:261
translate korean d01s02_30d051be:

    # mct "Hmmm ... how should I answer?"
    mct "흐으으음... 뭐라고 답해야 하지?"

# game/code/d01/s02.rpy:273
translate korean d01s02_2d4947fa:

    # mct "Hmmm ... it's not showing or doing anything."
    mct "으으으음... 뭘 보여주지도 않고 하지도 않네."

# game/code/d01/s02.rpy:275
translate korean d01s02_91e0cd29_1:

    # mct "Let's try to listen to the lecture."
    mct "강의나 들으려고 해야겠다."

# game/code/d01/s02.rpy:276
translate korean d01s02_025dccec:

    # "A couple minutes later."
    "몇 분 후."

# game/code/d01/s02.rpy:278
translate korean d01s02_295bbea7:

    # mct "..."
    mct "..."

# game/code/d01/s02.rpy:279
translate korean d01s02_99858e1d:

    # mct "Wha-a-at?"
    mct "무-어-야?"

# game/code/d01/s02.rpy:280
translate korean d01s02_48e2936f:

    # mct "What's going on with Antony? He's doing something strange. Am I the only one seeing this?!"
    mct "앤토니는 뭘 하는거야? 뭔가 이상한 짓을 하고 있는데. 이거 나만 보고 있는 건가?!"

# game/code/d01/s02.rpy:281
translate korean d01s02_56ac7286:

    # mct "I can't see enough from here. I need to move closer."
    mct "여기서는 잘 안 보이네. 좀 더 가까이 가야겠다."

# game/code/d01/s02.rpy:283
translate korean d01s02_e5ad10eb:

    # mc "Hey, Kev. It was nice talking to you, but I haven't talked to AmRose since I got back."
    mc "야, 케브. 오랜만에 이야기해서 좋았어. 그런데, 돌아오고 나서 앰로즈랑 이야기를 한 적이 없어서."

# game/code/d01/s02.rpy:284
translate korean d01s02_75652096:

    # kb "No problem! I'll see you tonight at the game?"
    kb "괜찮아! 저녁에 하는 농구 경기에서 볼래?"

# game/code/d01/s02.rpy:285
translate korean d01s02_ea86f4d4:

    # mc "Yeah, sure, see you there and thanks for the app. I'll continue to check it out."
    mc "그래, 좋아. 그럼 거기에서 보자. 그리고 앱 고마워. 계속 확인해 볼께"

# game/code/d01/s02.rpy:292
translate korean replay_d01s02_699878f6:

    # mc "Hey!"
    mc "어이!"

# game/code/d01/s02.rpy:294
translate korean replay_d01s02_4cd44184:

    # arj "..."
    arj "..."

# game/code/d01/s02.rpy:295
translate korean replay_d01s02_f3b3d28f:

    # mc "What's wrong?"
    mc "뭐가 문제야?"

# game/code/d01/s02.rpy:297
translate korean replay_d01s02_b199b51f:

    # arj "You were gone for two weeks and when you get back you don't even say Hi!"
    arj "너는 2주 동안 사라지곤 돌아와서는 안녕이라고 하지도 않는구나."

# game/code/d01/s02.rpy:300
translate korean replay_d01s02_ed0f5321:

    # mc "Ummmm."
    mc "으으으으음."

# game/code/d01/s02.rpy:302
translate korean replay_d01s02_3469a83b:

    # arj "Yeah, you were too distracted trying to hear what we were saying about Lydia."
    arj "그래 넌 우리가 리디아에 관해 이야기하는 걸 들으려고 정신이 팔렸었지."

# game/code/d01/s02.rpy:304
translate korean replay_d01s02_507086ef:

    # mc "Oh, that. Sorry."
    mc "아, 그건. 미안해."

# game/code/d01/s02.rpy:308
translate korean replay_d01s02_015d20de:

    # mct "Wow!"
    mct "와!"

# game/code/d01/s02.rpy:310
translate korean replay_d01s02_632ff841:

    # mct "Holy shit!"
    mct "씨발 세상에!"

# game/code/d01/s02.rpy:312
translate korean replay_d01s02_ab7d5066:

    # mct "I knew something was going on!"
    mct "뭔 짓을 하고 있는 줄 알았다니까!"

# game/code/d01/s02.rpy:315
translate korean replay_d01s02_dbccc912:

    # arj "Are you distracted again?"
    arj "넌 또 정신이 팔린 거니?"

# game/code/d01/s02.rpy:317
translate korean replay_d01s02_ed592207:

    # mc "No! Just trying to listen to the lecture, you know?"
    mc "아냐! 단지 강의를 들으려고 한 거라고, 알지?"

# game/code/d01/s02.rpy:319
translate korean replay_d01s02_36026660:

    # arj "C'mon, like, you never listen in history class."
    arj "그럴리가. 넌 역사 강의를 제대로 들은 적이 없잖아."

# game/code/d01/s02.rpy:321
translate korean replay_d01s02_d4c74386:

    # mc "True, but don't be so pissed. I brought you a movie, you are going to like it."
    mc "그렇긴 해. 그래도 너무 화내지 마. 내가 너 주려고 영화도 가져왔어. 너도 마음에 들 거야."

# game/code/d01/s02.rpy:323
translate korean replay_d01s02_84a64c74:

    # arj "Hmmm."
    arj "으으음."

# game/code/d01/s02.rpy:326
translate korean replay_d01s02_eaf4eca4:

    # arj "You know how to get out of trouble."
    arj "면피할 방법은 아는구나."

# game/code/d01/s02.rpy:327
translate korean replay_d01s02_80c2ad2e:

    # arj "What kind of movie is it?"
    arj "무슨 영환데?"

# game/code/d01/s02.rpy:332
translate korean replay_d01s02_0e7f0158:

    # mct "Damn!"
    mct "젠장!"

# game/code/d01/s02.rpy:333
translate korean replay_d01s02_9f24c4a8:

    # mct "I need to focus on what AmRose saying... {w}But I can't!"
    mct "앰로즈가 말하는 걸 집중해야 하는데... {w}집중할 수가 없어!"

# game/code/d01/s02.rpy:337
translate korean replay_d01s02_7ab8cb7f:

    # arj "Are you there?!"
    arj "여보세요?!"

# game/code/d01/s02.rpy:338
translate korean replay_d01s02_ee6bbee0:

    # mc "Yeah! Sure."
    mc "그래! 물론."

# game/code/d01/s02.rpy:340
translate korean replay_d01s02_3c7c87f9:

    # arj "What did I just ask you?"
    arj "내가 뭘 물어봤지?"

# game/code/d01/s02.rpy:346
translate korean replay_d01s02_b5006555:

    # mc "You asked about the movie!"
    mc "영화에 관해 물어봤잖아!"

# game/code/d01/s02.rpy:348
translate korean replay_d01s02_c48bdb4e:

    # mc "And I'm not gonna tell you what it's about. You'll like it, I'm sure."
    mc "하지만 난 무슨 영화인지 안 말해 줄 거야. 분명히 마음에 들어할 걸."

# game/code/d01/s02.rpy:352
translate korean replay_d01s02_7bb488f1:

    # mc "You were talking about me not listening History lectures?"
    mc "내가 역사 강의를 안 듣는다고 했지?"

# game/code/d01/s02.rpy:356
translate korean replay_d01s02_37f824cb:

    # mc "Me not being here for two weeks?"
    mc "2주 동안 내가 여기 없었던 거?"

# game/code/d01/s02.rpy:360
translate korean replay_d01s02_71b6273b:

    # arj "No! That's not what I asked."
    arj "아니! 그건 묻지 않았는데."

# game/code/d01/s02.rpy:363
translate korean replay_d01s02_4cd44184_1:

    # arj "..."
    arj "..."

# game/code/d01/s02.rpy:364
translate korean replay_d01s02_968cf464:

    # mct "Seems like I really pissed her off."
    mct "진짜 내가 얘를 화나게 한 모양이네."

# game/code/d01/s02.rpy:369
translate korean replay_d01s02_f9adf382:

    # arj "Hmmm. I might forgive you ... but only if we watch it together!"
    arj "으으으음. 용서를 해줄 수도 있어... 같이 영화를 본다면 말야!"

# game/code/d01/s02.rpy:370
translate korean replay_d01s02_15f6a903:

    # mc "Sure, we can do that!"
    mc "물론이지, 같이 영화 보자!"

# game/code/d01/s02.rpy:376
translate korean replay_d01s02_cf97b7f3:

    # mct "How is it possible that only I'm seeing it?! They're doing it right in the front row!"
    mct "어떻게 나만 이걸 볼 수 있는 거지?! 바로 맨 앞줄에서 이 짓을 하고 있는데!"

# game/code/d01/s02.rpy:377
translate korean replay_d01s02_bb084984:

    # mct "She going faster and faster."
    mct "민이 점점 빨라지는데."

# game/code/d01/s02.rpy:378
translate korean replay_d01s02_018ed043:

    # mct "How much longer can Antony last?"
    mct "앤토니가 얼마나 더 참을 수 있을까?"

# game/code/d01/s02.rpy:382
translate korean replay_d01s02_918fea83:

    # mc "I'm sorry."
    mc "미안해."

# game/code/d01/s02.rpy:383
translate korean replay_d01s02_786a7cd7:

    # mct "Looks like she isn't going to talk to me anymore."
    mct "앰로즈는 더 이상 나에게 말을 걸지 않을 모양이다."

# game/code/d01/s02.rpy:388
translate korean replay_d01s02_5c581103:

    # arj "Then it's a deal! We'll watch it tonight."
    arj "그럼 좋았어! 오늘 저녁에 보자."

# game/code/d01/s02.rpy:390
translate korean replay_d01s02_1e1e3d11:

    # mc "Tonight may be complicated ..."
    mc "저녁에는 좀 어려울 것 같은데..."

# game/code/d01/s02.rpy:393
translate korean replay_d01s02_06c4215c:

    # mc "I have to empty the trash tonight after the basketball game because Miss Waller saw a girl leaving our room this morning."
    mc "저녁에 농구 시합이 끝나면 쓰레기통을 비워야 하거든. 월러 사감이 오늘 아침에 우리 방에서 여재애가 나가는 걸 봤어."

# game/code/d01/s02.rpy:395
translate korean replay_d01s02_3b00d1ca:

    # arj "You brought a girl to your room?!"
    arj "여자애를 니 방에 들였다고?!"

# game/code/d01/s02.rpy:397
translate korean replay_d01s02_3a95b93e:

    # mc "No! Pete did."
    mc "내가 한 거 아냐! 피트가 그랬어."

# game/code/d01/s02.rpy:399
translate korean replay_d01s02_d527faec:

    # arj "Then why did you get in trouble?"
    arj "그럼 왜 니가 걸린 건데?"

# game/code/d01/s02.rpy:402
translate korean replay_d01s02_04c56411:

    # mc "We both got in trouble. Miss Waller didn't care who was responsible. She said because we share the room we share the responsibilities."
    mc "우리 둘 다 걸렸어. 월러 사감은 누구 책임인지는 신경도 안 썼어. 우리 둘이 방을 같이 쓰니까 책임도 같이 져야 한다고 하더라."

# game/code/d01/s02.rpy:404
translate korean replay_d01s02_23b83d84:

    # mc "I was just put in a bad spot and it isn't cool to rat out your friends."
    mc "나는 그냥 재수가 없었을 뿐이야. 그리고 친구를 고자질한다는 건 멋지지 않다고."

# game/code/d01/s02.rpy:406
translate korean replay_d01s02_bbfa59bc:

    # arj "Oh, so you're just being a good friend."
    arj "그래, 그렇게 해서 넌 단지 좋은 친구인 채로 있는 거지."

# game/code/d01/s02.rpy:408
translate korean replay_d01s02_46c4606b:

    # mc "I promised to hang with Pete tonight."
    mc "그리고 피터랑 저녁에 놀기로 약속했어."

# game/code/d01/s02.rpy:410
translate korean replay_d01s02_ccc69cb6:

    # arj "Hmmm. I see."
    arj "으으음. 알았어."

# game/code/d01/s02.rpy:420
translate korean replay_d01s02_e52ed3fc:

    # mct "Is he gonna finish now?!"
    mct "앤토니가 이제 싸는 건가?!"

# game/code/d01/s02.rpy:425
translate korean replay_d01s02_91b13306:

    # mct "Oh my God! He just came in her hand!"
    mct "오 이런 신이여! 앤토니가 방금 민의 손에 쌌다!"

# game/code/d01/s02.rpy:427
translate korean replay_d01s02_1ec6e58e:

    # mct "That's actually really hot!"
    mct "이거 정말 야한데!"

# game/code/d01/s02.rpy:432
translate korean replay_d01s02_4cd44184_2:

    # arj "..."
    arj "..."

# game/code/d01/s02.rpy:437
translate korean replay_d01s02_62c324fa:

    # mc "But we can watch it later ... after I'm finished."
    mc "그래도 나중에 같이 볼 수 있어 ... 내가 쓰레기통을 다 비운 다음에."

# game/code/d01/s02.rpy:439
translate korean replay_d01s02_b065aa01:

    # arj "Okay. that sounds cool."
    arj "좋아. 그거 좋은 걸."

# game/code/d01/s02.rpy:443
translate korean replay_d01s02_e7934d20:

    # mc "I already saw it anyway. I just wanted you to see it."
    mc "난 이미 그 영화를 보긴 했지만. 그래도 그냥 니 보면 좋겠더라."

# game/code/d01/s02.rpy:447
translate korean replay_d01s02_5f5a449a:

    # arj "I guess I should just be happy that you brought me something. I was hoping to do something together."
    arj "니가 나한테 뭘 가져다 준 것만으로도 기뻐해야 할 것 같네. 함께 같이 뭘 했으면 바랐거든."

# game/code/d01/s02.rpy:448
translate korean replay_d01s02_ca5f0fd9:

    # mct "Damn! She's acting so different! Does she wanna go out?!"
    mct "젠장! 평소랑 정말 다르게 행동하는데! 정말 나랑 같이 있고 싶은 건가?!"

# game/code/d01/s02.rpy:449
translate korean replay_d01s02_ad429af8:

    # mct "Her body language is definitely flirtatious!"
    mct "제 행동이 진짜 날 꼬시려는 거 같잖아!"

# game/code/d01/s02.rpy:455
translate korean replay_d01s02_4cd44184_3:

    # arj "..."
    arj "..."

# game/code/d01/s02.rpy:459
translate korean replay_d01s02_578606f6:

    # mct "Did she just swallow his whole load?"
    mct "방금 민이 앤토니가 싼 걸 다 삼킨 거야?"

# game/code/d01/s02.rpy:461
translate korean replay_d01s02_015d20de_1:

    # mct "Wow!"
    mct "와!"

# game/code/d01/s02.rpy:462
translate korean replay_d01s02_27953c55:

    # mct "That's ..."
    mct "이건..."

# game/code/d01/s02.rpy:463
translate korean replay_d01s02_468b4c54:

    # mct "That's probably the hottest thing I've seen in my life!"
    mct "이건 내가 이제껏 살면서 본 것 중에 가장 야한 걸 거야!"

# game/code/d01/s02.rpy:470
translate korean replay_d01s02_3090eac0:

    # arj "So, are you distracted again?"
    arj "그래, 또 다시 정신이 팔린 거야?"

# game/code/d01/s02.rpy:475
translate korean replay_d01s02_ee89dd2b:

    # mc "Sorry AmRose, I have to study for the exams."
    mc "미안 앰로즈, 시험 때문에 공부를 해야 해서."

# game/code/d01/s02.rpy:477
translate korean replay_d01s02_39681987:

    # arj "Okay. Sure, I understand."
    arj "좋아. 그래, 이해해."

# game/code/d01/s02.rpy:483
translate korean replay_d01s02_9879f0c8:

    # mc "Hmmm. I didn't know you could be so persuasive! Okay, let's do it tonight."
    mc "으으음. 니가 이렇게 잘 날 설득시킬지 몰랐는 걸. 그래 오늘 같이 영화 보자."

# game/code/d01/s02.rpy:484
translate korean replay_d01s02_8ba3d6f6:

    # arj "That sounds great."
    arj "그거 듣던 중 반가운 소식이네."

# game/code/d01/s02.rpy:488
translate korean replay_d01s02_a879797f:

    # mct "Okay. Seems like the lecture is almost over ..."
    mct "좋아. 강의가 거의 끝난 것 같은데..."

# game/code/d01/s02.rpy:489
translate korean replay_d01s02_300e3d8a:

    # mct "just a little longer."
    mct "이제 조금만 더."

# game/code/d01/s02.rpy:494
translate korean replay_d01s02_5c98fad5:

    # arjt "Hmmm."
    arjt "으으음."

# game/code/d01/s02.rpy:495
translate korean replay_d01s02_015d87b4:

    # arjt "What?!"
    arjt "뭐지?!"

# game/code/d01/s02.rpy:501
translate korean replay_d01s02_de6c46e2:

    # arjt "Oh wow!"
    arjt "오 와!"

# game/code/d01/s02.rpy:504
translate korean replay_d01s02_f5f3b3e1:

    # arjt "[mcname] got a hard-on! I was hoping that he would want to see the movie together but is he expecting something more than just a movie?!"
    arjt "[mcname]이/가 발기했어! 나는 그냥 같이 영화를 보길 바랐을 뿐인데, 영화 보는 이상의 걸 기대하는 거야?!"

# game/code/d01/s02.rpy:505
translate korean replay_d01s02_de19de29:

    # arjt "I'm sure he is. He just got super hard when I invited him."
    arjt "분명히 그런 거겠지. 영화를 같이 보자고 했는데 이렇게 엄청 딱딱해지다니."

# game/code/d01/s02.rpy:508
translate korean replay_d01s02_ae5cc99b:

    # arjt "What should I do?! We were friends for so long. He never showed any interest in me."
    arjt "어떻게 해야 하지?! 우린 정말 오랫동안 친구였잖아. 나한테는 털끝만큼도 관심이 없어 보였는데."

# game/code/d01/s02.rpy:509
translate korean replay_d01s02_037e9ef7:

    # arjt "And he's {b}SO{/b} into Lydia! Is he ready to just forget about her?! I'm {b}SO{/b} confused."
    arjt "그리고 얘는 리디아한테 {b}엄청{/b} 빠져 있잖아! 리디아를 잊으려고 하는 건가?! {b}정말{/b} 헷갈려."

# game/code/d01/s02.rpy:512
translate korean replay_d01s02_55d3d3cd:

    # mct "Oh man! I'm so hard after I saw how Min swallowed a handful of Antony's cum!"
    mct "이런 세상에! 민이 앤토니의 정액을 한 움쿰 삼키는 걸 보니까 정말 딱딱하게 섰어!"

# game/code/d01/s02.rpy:514
translate korean replay_d01s02_85246e7f:

    # mct "Oh man! I'm so hard after I saw Min jerking Antony off."
    mct "이런 세상에! 민이 앤토니를 대딸하는 걸 보니까 정말 딱딱하게 섰어."

# game/code/d01/s02.rpy:515
translate korean replay_d01s02_84d4c479:

    # mct "I hope I can walk out of class without someone noticing my hard dick."
    mct "내 좆이 선 걸 들키지 않고 강의실에서 나갈 수 있으면 좋겠는데."

# game/code/d01/s02.rpy:522
translate korean replay_d01s02_9096c34d:

    # mc "See ya tonight!"
    mc "오늘 저녁에 보자!"

# game/code/d01/s02.rpy:524
translate korean replay_d01s02_4c85d272:

    # mc "See ya!"
    mc "또 보자!"

# game/code/d01/s02.rpy:528
translate korean replay_d01s02_c484e8d4:

    # arj "Ciao!"
    arj "차우!"

translate korean strings:

    # game/code/d01/s02.rpy:262
    old "Yes, I like Cumshots"
    new "그럼, 나는 컴샷을 좋아해"

    # game/code/d01/s02.rpy:262
    old "No, Cumshot is not my fetish"
    new "아니, 컴샷은 내 페티시가 아냐"

    # game/code/d01/s02.rpy:343
    old "About the movie!"
    new "영화에 관해!"

    # game/code/d01/s02.rpy:343
    old "It was about listening to the lecture!"
    new "강의 듣는 것에 관한 거였지!"

    # game/code/d01/s02.rpy:343
    old "You asked why I was gone for two weeks!"
    new "내가 2주 동안 사라졌던 이유를 물어봤잖아!"

    # game/code/d01/s02.rpy:433
    old "See the movie after that"
    new "저녁에 일이 끝나면 영화를 본다"

    # game/code/d01/s02.rpy:433
    old "Don't go to see the movie with her"
    new "앰로즈와 영화를 보러 가지 않는다"

    # game/code/d01/s02.rpy:473
    old "Don't watch a movie with her"
    new "앰로즈와 영화를 보지 않는다"

    # game/code/d01/s02.rpy:473
    old "Change of plans! Let's see a movie with AmRose"
    new "계획 변경! 앰로즈와 함께 영화를 보자"

