# TODO: Translation updated at 2022-02-08 11:13

# game/code/d01/s04.rpy:112
translate korean s04_2433015c:

    # "6:00 PM"
    "오후 6:00"

# game/code/d01/s04.rpy:117
translate korean s04_0afa175b:

    # mct "There's Pete."
    mct "피트가 왔네."

# game/code/d01/s04.rpy:118
translate korean s04_669664cb:

    # mc "Hey."
    mc "어이."

# game/code/d01/s04.rpy:123
translate korean s04_e0ad8b16:

    # pb "Hey, there you are."
    pb "야, 여기 있었냐."

# game/code/d01/s04.rpy:126
translate korean s04_a60fb28f:

    # mc "Hey! Nice game! Congrats!"
    mc "야! 경기 좋았어! 잘했어!"

# game/code/d01/s04.rpy:128
translate korean s04_dd8f8ec0:

    # pb "Thanks. It was easy but the next game is gonna be a lot harder."
    pb "고마워. 오늘 경기는 쉬웠는데, 다음 경기는 더 어려울 것 같아."

# game/code/d01/s04.rpy:130
translate korean s04_7671f8f6:

    # mc "Then good luck with the next one."
    mc "그럼 다음 경기에 행운이 있긴 빈다."

# game/code/d01/s04.rpy:134
translate korean s04_723a5c65:

    # mc "I'm gonna start cleaning up. I wanna to finish this as quick as possible."
    mc "난 청소를 시작해야겠어. 최대한 빨리 끝내고 싶으니까."

# game/code/d01/s04.rpy:136
translate korean s04_589977f4:

    # pb "I'm here to help."
    pb "내가 도와줄께."

# game/code/d01/s04.rpy:137
translate korean s04_4a5e5694:

    # mc "???"
    mc "???"

# game/code/d01/s04.rpy:139
translate korean s04_63fa3f9f:

    # pb "I know Miss Waller got you for the girl I had in the room. There's no way I'm gonna let you do it alone."
    pb "내가 데려온 여자애 때문에 월러 사감이 청소 시킨 거 알아. 너 혼자 하게 둘 수 있겠냐."

# game/code/d01/s04.rpy:141
translate korean s04_5ea01134:

    # mc "Thanks! We'll finish this a lot quicker together."
    mc "고마워! 같이 하면 금방 끝낼 수 있을 거야."

# game/code/d01/s04.rpy:144
translate korean s04_08628ae2:

    # mc "Okay, let's get going."
    mc "좋아, 시작하자."

# game/code/d01/s04.rpy:146
translate korean s04_ce231830:

    # pb "Sorry for getting you in trouble too."
    pb "너도 엮기게 해서 미안해."

# game/code/d01/s04.rpy:148
translate korean s04_a3eeb706:

    # mc "Don't sweat it. Maybe next time I'll bring a girl and you'll have to help me clean."
    mc "그런 말 마. 다음번에 내가 여자애를 데려오면 니가 청소를 도와줘야 할 거니까."

# game/code/d01/s04.rpy:150
translate korean s04_9ed47da9:

    # pb "Hahaha! Right!"
    pb "하하하! 알았어!"

# game/code/d01/s04.rpy:152
translate korean s04_c707ac05:

    # mc "No. I'm serious."
    mc "야. 나 진지해."

# game/code/d01/s04.rpy:154
translate korean s04_62e10323:

    # pb "Sure."
    pb "그래."

# game/code/d01/s04.rpy:158
translate korean s04_05a0e74a:

    # pb "Sorry. I forgot to ask this morning, how's your [game.stacytitle]?"
    pb "미안. 아침에 물어보는 걸 깜빡했네. 니네 [game.stacytitle]은 어때?"

# game/code/d01/s04.rpy:160
translate korean s04_85e1c87a:

    # mc "She's doing great! She's changed a lot."
    mc "걔는 잘하고 있어! 많이 변했더라."

# game/code/d01/s04.rpy:162
translate korean s04_fc1333cc:

    # pb "How so?"
    pb "어떻게?"

# game/code/d01/s04.rpy:164
translate korean s04_785ba4a3:

    # mc "She used to be a nerdy, chubby girl the last time I saw her but now she's turned into this super slim sporty chick."
    mc "마지막에 봤을 때까지만 해도 너드에 통통한 애였는데, 지금은 날씬하고 발랄한 여자가 다 됐더라."

# game/code/d01/s04.rpy:166
translate korean s04_00aeba79:

    # pb "Wow! Sounds like a nice transformation. She isn't a computer nerd anymore?"
    pb "와! 괜찮게 변한 모양이네. 그럼 더는 컴퓨터 너드가 아닌 거야?"

# game/code/d01/s04.rpy:168
translate korean s04_7407f7c1:

    # mc "Oh, she is still super good with computers, but now she started running, is doing yoga and stuff like that. I don't know where she finds the time to do everything."
    mc "아, 걔는 여전히 컴퓨터를 엄청 잘 다뤄, 그렇지만 지금은 달리기도 시작했고, 요가 같은 것들을 하고 있더라. 그런 걸 다 할 시간을 어떻게 만들었나 몰라."

# game/code/d01/s04.rpy:170
translate korean s04_bb8ac82c:

    # pb "Cool. When she will be here?"
    pb "멋지네. 언제 온데?"

# game/code/d01/s04.rpy:172
translate korean s04_57c1172b:

    # mc "Hey! Keep your hands off my [game.stacytitle]! Don't even think about it!"
    mc "야! 내 [game.stacytitle]한테 손대지도 마! 손댈 생각도 하지 말라고!"

# game/code/d01/s04.rpy:174
translate korean s04_b6043fc1:

    # pb "Don't worry. I have enough pussy."
    pb "걱정마. 나랑 떡칠 애들은 충분하다고."

# game/code/d01/s04.rpy:176
translate korean s04_188e648b:

    # pb "But I can't promise I won't be thinking about her!"
    pb "그래도 생각하지도 않겠다는 약속은 못하겠네!"

# game/code/d01/s04.rpy:178
translate korean s04_e93ca3c5:

    # mc "Haha! You dirty prick!"
    mc "하하! 이 추잡한 새끼!"

# game/code/d01/s04.rpy:180
translate korean s04_add5108b:

    # pb "Guilty. But by your description of her, I'm pretty sure you thought about her yourself."
    pb "정답. 하지만 니 말을 들어보면, 니가 그런 생각을 하는 것 같은데."

# game/code/d01/s04.rpy:182
translate korean s04_aaadf451:

    # mc "What the fuck, dude? She's my [game.stacytitle]!"
    mc "새꺄, 뭐라고? 걔는 내 [game.stacytitle]이라고!"

# game/code/d01/s04.rpy:184
translate korean s04_8bacdffc:

    # pb "And I bet it didn't stop you from thinking about her hot ass."
    pb "그 사실이 니가 걔의 쩌는 엉덩이 생각하는 걸 막지는 못했을 걸."

# game/code/d01/s04.rpy:186
translate korean s04_57bcb3d0:

    # mct "I have to hand it to him, he's got a good point there."
    mct "그건 사실이었다고 인정해야겠는 걸."

# game/code/d01/s04.rpy:195
translate korean s04_bf4b3c03:

    # mct "It was a nice morning. Hung out with all my friends that are still in the city."
    mct "좋은 아침이야. 이 도시에 남아 있는 친구들이랑 잘 놀았어."

# game/code/d01/s04.rpy:197
translate korean s04_85bba9dd:

    # mct "Time to help Stacy get things packed for the trip back."
    mct "스테이시가 짐 싸는 걸 도와줄 시간이군."

# game/code/d01/s04.rpy:207
translate korean s04_015d20de:

    # mct "Wow!"
    mct "와!"

# game/code/d01/s04.rpy:208
translate korean s04_d20045c6:

    # mct "Should I watch her doing yoga?"
    mct "쟤가 요가하는 걸 봐야 하나?"

# game/code/d01/s04.rpy:227
translate korean s04_4a0540d1:

    # mct "Damn! My [game.stacytitle]'s so hot now!"
    mct "망할! 내 [game.stacytitle]이/가 이제는 너무 섹시해!"

# game/code/d01/s04.rpy:239
translate korean s04_a77fd7d6:

    # mct "Oh shit!"
    mct "오 젠장!"

# game/code/d01/s04.rpy:242
translate korean s04_b90f9b1f:

    # sy "Hey! How long you've been watching me?"
    sy "어이! 얼마나 오랫동안 날 보고 있던 거야?"

# game/code/d01/s04.rpy:243
translate korean s04_a8795464:

    # mc "I just got here."
    mc "나 방금 왔는데."

# game/code/d01/s04.rpy:244
translate korean s04_546ebee6:

    # mc "This used to be {i}my{/i} room ya know."
    mc "여기는 예전엔 {i}내{/i} 방이었다고, 알잖아."

# game/code/d01/s04.rpy:245
translate korean s04_50ed2d63:

    # sy "Well, it's not anymore."
    sy "글쎄, 이젠 아니지."

# game/code/d01/s04.rpy:246
translate korean s04_cd8fb47a:

    # mc "Old habits are hard to break."
    mc "오랜 습관은 고치기 어려워."

# game/code/d01/s04.rpy:247
translate korean s04_b13589e4:

    # sy "You still could have knocked."
    sy "그래도 노크는 할 수 있었잖아."

# game/code/d01/s04.rpy:248
translate korean s04_fab00458:

    # sy "Or were you actually trying to see me naked?"
    sy "아니면 진짜 내 알몸이라도 보려고 했던 거야?"

# game/code/d01/s04.rpy:249
translate korean s04_f2eaa3d2:

    # mc "You aren't naked!"
    mc "알몸 아니잖아!"

# game/code/d01/s04.rpy:253
translate korean s04_5bfce982:

    # sy "What if I'd been doing it naked? Would you still watch me?"
    sy "만약 내가 알몸이었다면? 그래도 계속 볼 꺼야?"

# game/code/d01/s04.rpy:255
translate korean s04_c16c614e:

    # mc "What?! Why would you be doing yoga naked?!"
    mc "뭐?! 왜 알몸으로 요가를 하는 거야?"

# game/code/d01/s04.rpy:257
translate korean s04_5eceac14:

    # sy "I do it sometimes when no ones home. It feels very natural."
    sy "집에 아무도 없으면, 가끔 그렇게 해. 엄청 자연스러운 느낌이 들거든."

# game/code/d01/s04.rpy:260
translate korean s04_53494d9e:

    # sy "Ahaha!"
    sy "아하하!"

# game/code/d01/s04.rpy:262
translate korean s04_68b5edbe:

    # sy "Ahaha! You dumbass."
    sy "아하하! 등신."

# game/code/d01/s04.rpy:263
translate korean s04_2b35b6fe:

    # mc "..."
    mc "..."

# game/code/d01/s04.rpy:266
translate korean s04_0d186106:

    # sy "Are you gonna help me pack these boxes or not?"
    sy "내가 짐 싸는 거 도와줄 거야 말 거야?"

# game/code/d01/s04.rpy:269
translate korean s04_51d6bfc1:

    # mct "What the hell?! I immediately got hard when she asked me that question, about watching her naked."
    mct "씨발 이게 뭐야?! 알몸이어도 볼 거냐고 물어보자마자 서다니."

# game/code/d01/s04.rpy:270
translate korean s04_cf53d3a1:

    # mct "She made it sound like it was a joke but was it?"
    mct "농담처럼 말하긴 했는데 진짜 농담인가?"

# game/code/d01/s04.rpy:282
translate korean s04_1d82d6f1:

    # pb "Hey?! You there?!"
    pb "어이?! 계세요?!"

# game/code/d01/s04.rpy:284
translate korean s04_e6592bd6:

    # mc "Yeah! No. I mean ... yeah, I'm here."
    mc "어! 아냐. 그러니까 내 말은... 어, 여기 있어."

# game/code/d01/s04.rpy:285
translate korean s04_589bedff:

    # mc "Okay, let's change the subject."
    mc "좋아, 화제를 바꾸자."

# game/code/d01/s04.rpy:286
translate korean s04_e06e17b7:

    # mc "Tell me about that girl from this morning. Where did you meet her?"
    mc "오늘 아침에 본 여자애에 관해서나 말해 봐. 어디서 만난 거야?"

# game/code/d01/s04.rpy:288
translate korean s04_ab758b66:

    # pb "Uhmm. I don't really know anything about her. I just met her at a party. She was looking to get laid and so I was happy to fuck her."
    pb "으음. 걔에 관해서는 진짜 아무것도 몰라. 파티에 만났어. 같이 자고 싶어해서 즐겁게 떡을 쳤지."

# game/code/d01/s04.rpy:290
translate korean s04_ca0ca19e:

    # pb "I thought we'd talk about Lydia. Isn't she the one you want?"
    pb "리디아에 관해서 이야기해야지. 리디아가 니가 빠진 걔지?"

# game/code/d01/s04.rpy:292
translate korean s04_2fbb7006:

    # mc "Absolutely."
    mc "맞아."

# game/code/d01/s04.rpy:294
translate korean s04_f77f1f18:

    # pb "I might have an idea about how you can hook up with her."
    pb "니가 걔랑 엮일 만한 건이 있을 거야."

# game/code/d01/s04.rpy:296
translate korean s04_28c73e5c:

    # mc "That's great! Hold that thought, I have to piss."
    mc "그거 좋은데! 잠깐만, 나 오줌 좀 싸야겠어."

# game/code/d01/s04.rpy:298
translate korean s04_62e10323_1:

    # pb "Sure."
    pb "그래."

# game/code/d01/s04.rpy:313
translate korean replay_d01s04_d355e38f:

    # "Klaaank"
    "꽝"

# game/code/d01/s04.rpy:314
translate korean replay_d01s04_9c132d58:

    # mct "Someone just came into the restroom."
    mct "누가 방금 화장실에 들어왔는데."

# game/code/d01/s04.rpy:315
translate korean replay_d01s04_3eef3a8e:

    # mct "It's already pretty late. No one should be here anymore."
    mct "꽤 늦었는데. 여기 있으면 안 되지 않나?"

# game/code/d01/s04.rpy:316
translate korean replay_d01s04_a31fe0ae:

    # "Girl voice" "Get in here!"
    "여자 목소리" "들어와!"

# game/code/d01/s04.rpy:317
translate korean replay_d01s04_6a2d8399:

    # pb "Hey chill! Are you in a hurry?"
    pb "야 진정해! 급하냐?"

# game/code/d01/s04.rpy:318
translate korean replay_d01s04_ef29e1d2:

    # mct "That's Pete's voice!"
    mct "이건 피트의 목소리잖아!"

# game/code/d01/s04.rpy:323
translate korean replay_d01s04_5cd1cde6:

    # mct "Oh my god! They are right next to me!"
    mct "오 이런 세상에! 쟤들이 내 바로 옆에 있어!"

# game/code/d01/s04.rpy:328
translate korean replay_d01s04_b695f088:

    # "Girl voice" "Shut the fuck up and give me your dick."
    "여자 목소리" "씨발 닥치고 좆이나 줘."

# game/code/d01/s04.rpy:329
translate korean replay_d01s04_b4ad5312:

    # mct "Hmm. The girl's voice sounds familiar too. Who is that?"
    mct "으음. 여자 목소리는 꽤 익숙한 걸. 누구지?"

# game/code/d01/s04.rpy:331
translate korean replay_d01s04_2fdfc77c:

    # pb "Okay-okay. Here you go. Choke on it bitch."
    pb "알았어-알았어. 여기 있어. 목이 멜 정도로 쑤셔 넣으라고, 썅년아."

# game/code/d01/s04.rpy:333
translate korean replay_d01s04_ad5ec6e0:

    # mct "What the fuck?! He knows that I just came into this restroom. Did he bring some girl here on purpose?"
    mct "씹 이게 뭐야?! 피트는 내가 방금 화장실에 들어온 걸 알잖아. 일부러 여자를 여기에 데려온 거야?"

# game/code/d01/s04.rpy:335
translate korean replay_d01s04_1d4de0ac:

    # pb "Yes! Suck it deep!"
    pb "그래! 깊숙이 빨아!"

# game/code/d01/s04.rpy:339
translate korean replay_d01s04_1a227f42:

    # "Girl voice" "Mfh-h-h..."
    "여자 목소리" "므프-으-으..."

# game/code/d01/s04.rpy:340
translate korean replay_d01s04_289df95c:

    # "Girl voice" "Umgh..."
    "여자 목소리" "음흐..."

# game/code/d01/s04.rpy:342
translate korean replay_d01s04_1c3e2998:

    # "Girl voice" "Just cum faster! I need your jizz!"
    "여자 목소리" "그냥 빨리 싸라고! 니 좆물이 필요하단 말야!"

# game/code/d01/s04.rpy:343
translate korean replay_d01s04_dc2d9f45:

    # mct "Okay, I have to see this."
    mct "좋아, 봐야겠어."

# game/code/d01/s04.rpy:346
translate korean replay_d01s04_85581721:

    # mct "Holy shit! It's Chloe!"
    mct "씨발 세상에! 클로이잖아!"

# game/code/d01/s04.rpy:350
translate korean replay_d01s04_0fcc45f8:

    # pb "Ohhh! There you go!"
    pb "오오오! 온다!"

# game/code/d01/s04.rpy:351
translate korean replay_d01s04_fffb037c:

    # pb "Suck it good and deep!"
    pb "빨아 좋아 깊이!"

# game/code/d01/s04.rpy:352
translate korean replay_d01s04_3521c34c:

    # pb "Yeah, bitch! Where do you want me to cum?"
    pb "그렇지, 썅년아! 어디에 싸길 바라?"

# game/code/d01/s04.rpy:353
translate korean replay_d01s04_ba0e7011:

    # cb "Ugh-tugh-tugh."
    cb "우흐-트흐-트흐."

# game/code/d01/s04.rpy:354
translate korean replay_d01s04_3ccb40c9:

    # pb "What?"
    pb "뭐라고?"

# game/code/d01/s04.rpy:359
translate korean replay_d01s04_660ea8f8:

    # cb "Tits! Cum on my tits please!"
    cb "젖! 내 젖에 싸 줘!"

# game/code/d01/s04.rpy:360
translate korean replay_d01s04_67128b74:

    # pb "You're gonna have to suck it better bitch!"
    pb "좀 더 잘 빨아야 할 거야!"

# game/code/d01/s04.rpy:365
translate korean replay_d01s04_96f4cb40:

    # cb "Mfghhh-Mfghhh!"
    cb "음프으으으-음프으으으!"

# game/code/d01/s04.rpy:366
translate korean replay_d01s04_3a55d13a:

    # cb "Slurp... Slurp..."
    cb "꿀꺽... 꿀꺽..."

# game/code/d01/s04.rpy:367
translate korean replay_d01s04_f0155650:

    # pb "Oh yea, I'm close!"
    pb "오 그래, 싼다!"

# game/code/d01/s04.rpy:372
translate korean replay_d01s04_2b2147b8:

    # cb "Do it! Cum on me!"
    cb "싸줘! 나한테 싸달라고!"

# game/code/d01/s04.rpy:373
translate korean replay_d01s04_fc3ddb94:

    # cb "Cover my tits with your cum!"
    cb "내 젖을 니 정액으로 덮어 줘!"

# game/code/d01/s04.rpy:376
translate korean replay_d01s04_b4a79b8f:

    # pb "Oooh! Yes!"
    pb "오오오! 그래!"

# game/code/d01/s04.rpy:377
translate korean replay_d01s04_6524e1f1:

    # cb "Finally."
    cb "드디어."

# game/code/d01/s04.rpy:378
translate korean replay_d01s04_015d20de:

    # mct "Wow!"
    mct "와우!"

# game/code/d01/s04.rpy:380
translate korean replay_d01s04_63bfd6e4:

    # mct "Of course he knows I'm here!"
    mct "그럼 그렇지 내가 있는 걸 알고 있잖아!"

# game/code/d01/s04.rpy:382
translate korean replay_d01s04_47895ca8:

    # cb "Now take a picture."
    cb "이제 사진 찍어."

# game/code/d01/s04.rpy:383
translate korean replay_d01s04_a651910b:

    # mct "She wants a picture. This is for the challenge on the app."
    mct "클로이는 사진을 원하는 거야. 앱 도전 때문에 이런 거구나."

# game/code/d01/s04.rpy:384
translate korean replay_d01s04_2b354a63:

    # mct "Pete's taking a picture. I can take one too. Should I do it?"
    mct "피트가 사진을 찍는다. 나도 사진을 찍을 수 있겠네. 그렇게 해야 할까?"

# game/code/d01/s04.rpy:389
translate korean replay_d01s04_3bb39644:

    # mct "Okay, let's do it."
    mct "좋아, 찍자."

# game/code/d01/s04.rpy:395
translate korean replay_d01s04_5030c35c:

    # mct "Nice. Maybe I can use this in the future."
    mct "좋았어. 나중에 이 사진을 쓸 수 있을 거야."

# game/code/d01/s04.rpy:398
translate korean replay_d01s04_7dc3ebcb:

    # mct "It's a bad idea."
    mct "이건 나쁜 생각이야."

# game/code/d01/s04.rpy:401
translate korean replay_d01s04_d464aeeb:

    # mct "I should wait till they are gone and head back."
    mct "쟤들이 갈 때까지 기다렸다가 나가야 돼."

# game/code/d01/s04.rpy:410
translate korean replay_d01s04_cb0985d9:

    # pb "Did you like the show?"
    pb "쇼는 마음에 들었냐?"

# game/code/d01/s04.rpy:412
translate korean replay_d01s04_2e7638aa:

    # mc "Dude! Why Chloe?!"
    mc "새꺄! 왜 클로이야?!"

# game/code/d01/s04.rpy:414
translate korean replay_d01s04_403dad95:

    # pb "She was hungry for some cum and she knows I always have some to give."
    pb "걔가 좆물을 고파 했거든 그리고 걔는 내가 항상 그걸 줄 수 있다는 걸 알지."

# game/code/d01/s04.rpy:416
translate korean replay_d01s04_2a539ab8:

    # mc "Is it because of that new app?"
    mc "그 새로운 앱 때문이야?"

# game/code/d01/s04.rpy:418
translate korean replay_d01s04_4ed58fa4:

    # pb "Ah, you already heard about it. Yeah, she desperately wants some points!"
    pb "아, 너도 이미 들었구나. 그래, 걔는 점수 좀 벌려고 혈안이 됐더라."

# game/code/d01/s04.rpy:420
translate korean replay_d01s04_d1bff6d8:

    # mc "But I thought that the winner is already selected for today?!"
    mc "그래도 나는 오늘의 승자는 이미 정해졌다고 생각했는데?"

# game/code/d01/s04.rpy:422
translate korean replay_d01s04_0155661a:

    # pb "I see you already know a lot about Fetish Locator!"
    pb "너 이미 페티시 로케이터에 관해 많이 알고 있구나!"

# game/code/d01/s04.rpy:424
translate korean replay_d01s04_361d9674:

    # pb "Yes, you are right, the fetish of the day already has a winner. It was actually announced pretty early today."
    pb "그래, 니가 맞아, 오늘의 페티시 승자는 이미 정해졌지. 사실 오늘은 꽤 빨리 정해진 편이야."

# game/code/d01/s04.rpy:425
translate korean replay_d01s04_f65bbde0:

    # pb "But you can always get some points for uploading some hot stuff to the app. It just has to be connected with the fetish of the day."
    pb "그래도 앱에 핫한 걸 올리면 항상 점수를 벌 수 있어. 오늘의 페티시랑 관련만 있으면 말야."

# game/code/d01/s04.rpy:427
translate korean replay_d01s04_53139d4d:

    # mc "Oh okay, I see."
    mc "아 그렇구나, 알았어."

# game/code/d01/s04.rpy:429
translate korean replay_d01s04_c74d9f3c:

    # pb "That's what I wanted to propose to you."
    pb "이게 내가 너한테 제안하고 싶었던 거야."

# game/code/d01/s04.rpy:431
translate korean replay_d01s04_81737331:

    # mc "To use the app?"
    mc "앱을 쓰는 거 말이야?"

# game/code/d01/s04.rpy:433
translate korean replay_d01s04_c001942c:

    # pb "Well not only that. If you open the app there's a shop with stuff you can buy for points."
    pb "글쌔 그거뿐은 아니고. 앱을 열면 점수로 살 수 있는 게 있는 상점이 있어."

# game/code/d01/s04.rpy:434
translate korean replay_d01s04_d18429d4:

    # pb "One of the things on it is a ticket to a party. It'll be in two days and I got info for you that Lydia will be there."
    pb "거기서 살 수 있는 것 중 하나가 파티에 참석할 수 있는 표야. 이틀 뒤면 파티가 열릴 거고, 그 파티에 리디아도 올 거라는 정보도 입수했지."

# game/code/d01/s04.rpy:436
translate korean replay_d01s04_9e891473:

    # mc "She's using the app too?!"
    mc "리디아도 이 앱을 쓰는 거야?!"

# game/code/d01/s04.rpy:438
translate korean replay_d01s04_5c557e0e:

    # pb "I'm sure she's probably using it."
    pb "분명히 걔도 이 앱을 쓰고 있을 걸."

# game/code/d01/s04.rpy:440
translate korean replay_d01s04_239f55bf:

    # mc "Are you sure? She doesn't seem like someone who would use it."
    mc "너 확실해? 리디아는 이런 앱을 쓰는 사람 같지는 않은데."

# game/code/d01/s04.rpy:442
translate korean replay_d01s04_552f7159:

    # pb "I think here at this school everyone is using the app."
    pb "이 학교에서는 다 이 앱을 쓰는 것 같던데."

# game/code/d01/s04.rpy:443
translate korean replay_d01s04_94cfc009:

    # pb "And I trust my source. App or not she'll be at the party!"
    pb "그리고 나는 내 정보원을 믿어. 앱이든 아니든 걔는 파티에 갈 거야!"

# game/code/d01/s04.rpy:447
translate korean replay_d01s04_c6883085:

    # mc "Wow the ticket costs 50 points!"
    mc "와 표는 50점이나 하잖아!"

# game/code/d01/s04.rpy:448
translate korean replay_d01s04_dbdea09a:

    # mc "That's the same amount that someone won today."
    mc "오늘 승자가 딴 점수랑 같은 점수잖아."

# game/code/d01/s04.rpy:450
translate korean replay_d01s04_2da0ad6e:

    # mct "I probably shouldn't tell Pete I know who won those points."
    mct "그 점수를 딴 사람이 누구인지 알고 있다고 피트에게는 말하지 말아야겠지."

# game/code/d01/s04.rpy:453
translate korean replay_d01s04_5410f6a9:

    # pb "Those were winners of the challenge. They award points once a day."
    pb "걔들이 오늘 도전의 승자잖아. 하루 한 건씩 점수를 받을 수 있다고."

# game/code/d01/s04.rpy:455
translate korean replay_d01s04_792c5199:

    # pb "You don't have to win the challenge. You can just submit something interesting and get points just not as many as if you would win the challenge."
    pb "도전에서 이길 필요는 없어. 그냥 재미있는 걸 올리면 돼 그럼 점수를 딸 수 있어. 도전에서 이긴 만큼은 아니겠지만."

# game/code/d01/s04.rpy:457
translate korean replay_d01s04_b7b3dc33:

    # mc "Why can't I just buy some points?"
    mc "그냥 점수를 사면 되는 거 아냐?"

# game/code/d01/s04.rpy:459
translate korean replay_d01s04_0376ba43:

    # pb "You can, but to buy the ticket at least half of the points have to be earned and not bought."
    pb "그렇게 할 수 있긴 하지만, 표를 사려면 적어도 그 점수 절반은 사는 게 아니라 니가 직접 따야 돼."

# game/code/d01/s04.rpy:461
translate korean replay_d01s04_7a9ceb96:

    # mc "Hmmm strange."
    mc "으으음 이상하네."

# game/code/d01/s04.rpy:463
translate korean replay_d01s04_bb29afd0:

    # pb "Yeah, so this is the plan - you use the app, get the ticket and meet Lydia at the party."
    pb "그렇지, 그래 이게 계획이야 - 앱을 쓴다, 표를 산다, 파티에서 리디아를 만난다."

# game/code/d01/s04.rpy:465
translate korean replay_d01s04_fb2526d3:

    # mc "I see. Nothing could go wrong with that plan!"
    mc "알겠어. 이 계획이면 아무 문제도 없을 거야!"

# game/code/d01/s04.rpy:467
translate korean replay_d01s04_dea4d98c:

    # pb "Exactly!"
    pb "정확해!"

# game/code/d01/s04.rpy:469
translate korean replay_d01s04_5b237915:

    # pb "I gotta run. There's some celebration planned for the team. Later!"
    pb "난 가야겠다. 팀을 위한 축하 행사 같은 게 있거든. 나중에 보자!"

# game/code/d01/s04.rpy:471
translate korean replay_d01s04_16a73af2:

    # mc "Later!"
    mc "나중애 봐!"

# game/code/d01/s04.rpy:475
translate korean replay_d01s04_ff2ff749:

    # mct "This plan is stupid, but what do I have to lose for trying?"
    mct "바보 같은 계획이긴 하지만, 해봐서 손해 볼 건 없지."

# game/code/d01/s04.rpy:476
translate korean replay_d01s04_2e86dd70:

    # mct "The party's in two days. I don't have much time to earn the points."
    mct "파티는 이틀 뒤야. 점수 딸 시간이 많이는 없어."

# game/code/d01/s04.rpy:478
translate korean replay_d01s04_bdaff425:

    # mct "Today's fetish is Cumshot. As far as I understood it, I have to submit something related to that."
    mct "오늘의 페티시는 컴샷이야. 내가 알기론, 이거랑 관련된 건 올려야 돼."

# game/code/d01/s04.rpy:479
translate korean replay_d01s04_18c318ba:

    # mct "Just jerking off in shower isn't going to cut it. I guess it needs to be something ... impressive."
    mct "그냥 샤워 중에 딸치는 걸로는 충분하지 않을 거야. 뭔가... 인상적인 게 필요할텐데."

# game/code/d01/s04.rpy:483
translate korean replay_d01s04_325c4b97:

    # mct "I promised to watch a movie with AmRose. I need to do something to earn the points. But if I don't go with AmRose she'll be really pissed."
    mct "앰로즈랑 영화 보기로 약속했었지. 점수를 따려면 뭔갈 해야 해. 하지만 앰로즈랑 같이 영화를 보지 않으면 엄청 화낼텐데."

translate korean strings:

    # game/code/d01/s04.rpy:210
    old "That is my [game.stacytitle]! Should give her some privacy"
    new "얘는 내 [game.stacytitle]이야! 사생활을 지켜 줘야지"

    # game/code/d01/s04.rpy:210
    old "Should watch more"
    new "더 봐야 해"

    # game/code/d01/s04.rpy:386
    old "Take a picture of Chloe"
    new "클로이의 사진을 찍는다"

    # game/code/d01/s04.rpy:386
    old "Or don't do it"
    new "아냐 하지 말자"

    # game/code/d01/s04.rpy:485
    old "Go to AmRose as you promised her to see a movie together"
    new "약속한 대로 영화를 같이 보기 위해 앰로즈에게 간다"

    # game/code/d01/s04.rpy:485
    old "Don't go to AmRose and try to earn some points"
    new "앰로즈에게 가지 않고 점수를 따려고 한다"

