translate korean strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Min & Antony"
    new "1일차: 민 & 앤토니"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to cumshots"
    new "컴샷을 수락한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Chloe & Peter"
    new "1일차: 클로이 & 피터"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Stacy (flashback)"
    new "2일차: 스테이시 (회상)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Nora & Polly"
    new "2일차: 노라 & 폴리"

    # game/code/data/extended/replay_room.rpy:15
    old "You must have posted your lips, met Polly and accepted her conditions"
    new "입술 사진을 올리고, 폴리를 만나서 그녀의 조건에 동의한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Min"
    new "2일차: 민"

    # game/code/data/extended/replay_room.rpy:15
    old "Make Min cum and keep watching her until the end"
    new "민을 싸게 만들고 끝까지 본다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - Nora route"
    new "2일차: 폴리 - 노라 경로"

    # game/code/data/extended/replay_room.rpy:15
    old "Assfinger Polly"
    new "폴리의 엉덩이를 손가락으로 쑤신다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - solo"
    new "3일차: 폴리 - 혼자"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Stacy (flashback)"
    new "3일차: 스테이시 (회상)"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't scare Stacy"
    new "스테이시를 놀라게 하지 않는다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Chloe (NTR)"
    new "3일차: 클로이 (NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Save Kevin"
    new "케빈을 돕는다"

    # game/code/data/extended/replay_room.rpy:15
    old "Submit to Dahlia"
    new "달리아에게 복종한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Secret Santa"
    new "파티: 시크릿 산타"

    # game/code/data/extended/replay_room.rpy:15
    old "Choose the event and request to see Lyssa's pussy"
    new "해당 행사를 고르고 리사에게 보지를 보여 달라고 요구한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: QueenB"
    new "파티: 퀸비"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle's event"
    new "파티: 스퀴즐의 행사"

    # game/code/data/extended/replay_room.rpy:15
    old "Do whatever it takes to win"
    new "이기기 위해 뭐든지 한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Mr.D1cky's event"
    new "파티: 미스터 ㄷI키의 행사"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Allison & Vesna"
    new "파티: 앨리슨 & 베스나"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle & Mr.D1cky"
    new "파티: 스퀴즐 & 미스터 ㄷI키"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Daisy"
    new "4일차: 데이지"

    # game/code/data/extended/replay_room.rpy:15
    old "Command Daisy to blow you (you must have refused Dahlia's request)"
    new "데이지에게 빨으라고 명령한다 (달리아의 요구를 거절해야 한다)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: AmRose"
    new "4일차: 앰로즈"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to enema play"
    new "관장 플레이를 수락한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Polly & Nora"
    new "4일차: 폴리 & 노라"

    # game/code/data/extended/replay_room.rpy:15
    old "Finger Polly (any hole)"
    new "폴리를 손가락으로 쑤신다 (어느 구멍이든)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Chloe"
    new "5일차: 클로이"

    # game/code/data/extended/replay_room.rpy:15
    old "It's showtime! (available if you have supported Kevin's views on NTR)"
    new "쇼타임! (NTR에 관한 케빈의 생각을 지지하는 경우)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Polly"
    new "5일차: 폴리"

    # game/code/data/extended/replay_room.rpy:15
    old "Strip and join Polly"
    new "벗고 폴리와 함께 한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URLittlePony"
    new "5일차: 유어리틀포니"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Squizzle"
    new "5일차: 스퀴즐"

    # game/code/data/extended/replay_room.rpy:15
    old "Earn a gold star"
    new "황금 별을 획득한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Daisy"
    new "5일차: 데이지"

    # game/code/data/extended/replay_room.rpy:15
    old "Invite Daisy and don't be shy"
    new "데이지를 초대하고 부끄러워 하지 않는다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Stacy (flashback)"
    new "5일차: 스테이시 (회상)"

    # game/code/data/extended/replay_room.rpy:15
    old "Jerk off"
    new "딸 친다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Daisy"
    new "6일차: 데이지"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Daisy"
    new "데이지에게 크림파이를 한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Jessie (dorm)"
    new "6일차: 제시 (기숙사)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Lydia"
    new "6일차: 리디아"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Nora"
    new "노라에게 크림파이를 한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Polly & Nora"
    new "6일차: 폴리 & 노라"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked if you are not romancing Polly"
    new "폴리와 사귀지 않으면 해금된다"

    # game/code/data/extended/replay_room.rpy:15
    old "Take a more dominant role"
    new "더욱 지배적인 역할을 맡는다 "

    # game/code/data/extended/replay_room.rpy:15
    old "Cum on Vanessa's feet"
    new "버네사의  발에 사정한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask for a release"
    new "해소를 요구한다"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Maria & AmRose (flashback)"
    new "7일차: 마리아 & 앰로즈 (회상)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Dahlia"
    new "7일차: 달리아"

    # game/code/data/extended/replay_room.rpy:15
    old "Spank Lyssa twelve times"
    new "12번 리사를 손바닥으로 때린다"

    # game/code/data/extended/replay_room.rpy:15
    old "Enjoy the rimjob"
    new "림잡을 즐긴다"

