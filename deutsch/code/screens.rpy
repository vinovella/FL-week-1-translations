# TODO: Translation updated at 2019-12-10 12:02

translate deutsch_hidden strings:

    # game/screens.rpy:256
    old "Back"
    new "Zurück"

    # game/screens.rpy:257
    old "History"
    new "Verlauf"

    # game/screens.rpy:258
    old "Skip"
    new "Überspringen"

    # game/screens.rpy:259
    old "Auto"
    new "Auto"

    # game/screens.rpy:260
    old "Save"
    new "Speichern"

    # game/screens.rpy:261
    old "Q.Save"
    new "S.speichern"

    # game/screens.rpy:262
    old "Q.Load"
    new "S.laden"

    # game/screens.rpy:263
    old "Prefs"
    new "Einst."

    # game/screens.rpy:312
    old "Load"
    new "Laden"

    # game/screens.rpy:314
    old "Preferences"
    new "Einstellungen"

    # game/screens.rpy:316
    old "Support on Patreon"
    new "Auf Patreon unterstützen"

    # game/screens.rpy:318
    old "Join Discord"
    new "Discord beitreten"

    # game/screens.rpy:322
    old "End Replay"
    new "Wiederholung beenden"

    # game/screens.rpy:326
    old "Main Menu"
    new "Hauptmenü"

    # game/screens.rpy:328
    old "About"
    new "Über"

    # game/screens.rpy:333
    old "Help"
    new "Hilfe"

    # game/screens.rpy:336
    old "Quit"
    new "Verlassen"

    # game/screens.rpy:567
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Hergestellt mit {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:607
    old "Page {}"
    new "Seite {}"

    # game/screens.rpy:607
    old "Automatic saves"
    new "Auto Speicherungen"

    # game/screens.rpy:607
    old "Quick saves"
    new "Schnell Speicherungen"

    # game/screens.rpy:649
    old "empty slot"
    new "leerer Platz"

    # game/screens.rpy:735
    old "Display"
    new "Anzeige"

    # game/screens.rpy:736
    old "Window"
    new "Fenster"

    # game/screens.rpy:737
    old "Fullscreen"
    new "Vollbild"

    # game/screens.rpy:741
    old "Rollback Side"
    new "Rollback Seite"

    # game/screens.rpy:742
    old "Disable"
    new "Deaktivieren"

    # game/screens.rpy:743
    old "Left"
    new "Links"

    # game/screens.rpy:744
    old "Right"
    new "Rechts"

    # game/screens.rpy:749
    old "Unseen Text"
    new "Ungesehener Text"

    # game/screens.rpy:750
    old "After Choices"
    new "Nach Entscheidungen"

    # game/screens.rpy:751
    old "Transitions"
    new "Übergänge"

    # game/screens.rpy:768
    old "Text Speed"
    new "Text Geschwindigkeit"

    # game/screens.rpy:772
    old "Auto-Forward Time"
    new "Auto-Vorspulzeit"

    # game/screens.rpy:779
    old "Music Volume"
    new "Musiklautstärke"

    # game/screens.rpy:786
    old "Sound Volume"
    new "Tonlautstärke"

    # game/screens.rpy:796
    old "Voice Volume"
    new "Sprachlautstärke"

    # game/screens.rpy:807
    old "Mute All"
    new "Alles stummschalten"

    # game/screens.rpy:926
    old "The dialogue history is empty."
    new "Der Dialogverlauf ist leer."

    # game/screens.rpy:996
    old "Keyboard"
    new "Tastatur"

    # game/screens.rpy:997
    old "Mouse"
    new "Maus"

    # game/screens.rpy:1014
    old "Advances dialogue and activates the interface."
    new "Führt den Dialog fort und aktiviert die Benutzeroberfläche."

    # game/screens.rpy:1017
    old "Space"
    new "Leertaste"

    # game/screens.rpy:1018
    old "Advances dialogue without selecting choices."
    new "Führt den Dialog fort, ohne Entscheidungen auszuwählen."

    # game/screens.rpy:1021
    old "Arrow Keys"
    new "Pfeiltasten"

    # game/screens.rpy:1022
    old "Navigate the interface."
    new "Steuert die Benutzeroberfläche."

    # game/screens.rpy:1025
    old "Escape"
    new "ESC"

    # game/screens.rpy:1026
    old "Accesses the game menu."
    new "Ruft das Spielmenü auf."

    # game/screens.rpy:1029
    old "Ctrl"
    new "Strg"

    # game/screens.rpy:1030
    old "Skips dialogue while held down."
    new "Überspringt den Dialog wenn gedrückt."

    # game/screens.rpy:1034
    old "Toggles dialogue skipping."
    new "Schaltet das Überspringen des Dialogs ein/aus."

    # game/screens.rpy:1037
    old "Page Up"
    new "Bild-Auf"

    # game/screens.rpy:1038
    old "Rolls back to earlier dialogue."
    new "Scrollt zurück zu früheren Dialog."

    # game/screens.rpy:1041
    old "Page Down"
    new "Bild-Ab"

    # game/screens.rpy:1042
    old "Rolls forward to later dialogue."
    new "Scrollt nach vorne zu späteren Dialog."

    # game/screens.rpy:1046
    old "Hides the user interface."
    new "Versteckt die Benutzeroberfläche."

    # game/screens.rpy:1050
    old "Takes a screenshot."
    new "Macht einen Screenshot."

    # game/screens.rpy:1054
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "Schaltet assistive {a=https://www.renpy.org/l/voicing}Sprachausgabe{/a} ein/aus."

    # game/screens.rpy:1060
    old "Left Click"
    new "Linksklick"

    # game/screens.rpy:1064
    old "Middle Click"
    new "Mittlere Maustaste"

    # game/screens.rpy:1068
    old "Right Click"
    new "Rechtsklick"

    # game/screens.rpy:1072
    old "Mouse Wheel Up\nClick Rollback Side"
    new "Mausrad nach oben\nKlick Rollback Seite"

    # game/screens.rpy:1076
    old "Mouse Wheel Down"
    new "Mausrad nach unten"

    # game/screens.rpy:1083
    old "Right Trigger\nA/Bottom Button"
    new "Rechter Trigger\nA/Untere Taste"

    # game/screens.rpy:1087
    old "Left Trigger\nLeft Shoulder"
    new "Linker Trigger\nLinke Schulter"

    # game/screens.rpy:1091
    old "Right Shoulder"
    new "Rechte Schulter"

    # game/screens.rpy:1104
    old "Y/Top Button"
    new "Y/Obere Taste"

    # game/screens.rpy:1107
    old "Calibrate"
    new "Kalibrieren"

    # game/screens.rpy:1172
    old "Yes"
    new "Ja"

    # game/screens.rpy:1173
    old "No"
    new "Nein"

    # game/screens.rpy:1219
    old "Skipping"
    new "Überspringen"

    # game/screens.rpy:1442
    old "Menu"
    new "Menü"
# TODO: Translation updated at 2020-01-16 00:03

translate deutsch strings:

    # game/screens.rpy:256
    old "Back"
    new "Zurück"

    # game/screens.rpy:257
    old "History"
    new "Verlauf"

    # game/screens.rpy:258
    old "Skip"
    new "Überspringen"

    # game/screens.rpy:259
    old "Auto"
    new "Auto"

    # game/screens.rpy:260
    old "Save"
    new "Speichern"

    # game/screens.rpy:261
    old "Q.Save"
    new "S.Speichern"

    # game/screens.rpy:262
    old "Q.Load"
    new "S.Laden"

    # game/screens.rpy:263
    old "Prefs"
    new "Einst."

    # game/screens.rpy:304
    old "Start"
    new "Start"

    # game/screens.rpy:312
    old "Load"
    new "Laden"

    # game/screens.rpy:314
    old "Preferences"
    new "Einstellungen"

    # game/screens.rpy:316
    old "Support on Patreon"
    new "Auf Patreon unterstützen"

    # game/screens.rpy:318
    old "Join Discord"
    new "Discord beitreten"

    # game/screens.rpy:320
    old "Switch Language"
    new "Sprache ändern"

    # game/screens.rpy:324
    old "End Replay"
    new "Replay beenden"

    # game/screens.rpy:328
    old "Main Menu"
    new "Hauptmenü"

    # game/screens.rpy:330
    old "About"
    new "Über"

    # game/screens.rpy:335
    old "Help"
    new "Hilfe"

    # game/screens.rpy:338
    old "Quit"
    new "Verlassen"

    # game/screens.rpy:479
    old "Return"
    new "Zurück"

    # game/screens.rpy:563
    old "Version [config.version!t]\n"
    new "Version [config.version!t]\n"

    # game/screens.rpy:569
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Hergestellt mit {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:609
    old "Page {}"
    new "Seite {}"

    # game/screens.rpy:609
    old "Automatic saves"
    new "Automatische Speicherungen"

    # game/screens.rpy:609
    old "Quick saves"
    new "Schnell Speicherungen"

    # game/screens.rpy:651
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%A, %B %d %Y, %H:%M"

    # game/screens.rpy:651
    old "empty slot"
    new "leerer Platz"

    # game/screens.rpy:668
    old "<"
    new "<"

    # game/screens.rpy:671
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:674
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:680
    old ">"
    new ">"

    # game/screens.rpy:737
    old "Display"
    new "Anzeige"

    # game/screens.rpy:738
    old "Window"
    new "Fenster"

    # game/screens.rpy:739
    old "Fullscreen"
    new "Vollbild"

    # game/screens.rpy:743
    old "Rollback Side"
    new "Rollback Seite"

    # game/screens.rpy:744
    old "Disable"
    new "Deaktivieren"

    # game/screens.rpy:745
    old "Left"
    new "Links"

    # game/screens.rpy:746
    old "Right"
    new "Rechts"

    # game/screens.rpy:751
    old "Unseen Text"
    new "Ungesehener Text"

    # game/screens.rpy:752
    old "After Choices"
    new "Nach Entscheidungen"

    # game/screens.rpy:753
    old "Transitions"
    new "Übergänge"

    # game/screens.rpy:770
    old "Text Speed"
    new "Text Geschwindigkeit"

    # game/screens.rpy:774
    old "Auto-Forward Time"
    new "Auto-Vorspulzeit"

    # game/screens.rpy:781
    old "Music Volume"
    new "Musiklautstärke"

    # game/screens.rpy:788
    old "Sound Volume"
    new "Tonlautstärke"

    # game/screens.rpy:794
    old "Test"
    new "Test"

    # game/screens.rpy:798
    old "Voice Volume"
    new "Sprachlautstärke"

    # game/screens.rpy:809
    old "Mute All"
    new "Alles stummschalten"

    # game/screens.rpy:928
    old "The dialogue history is empty."
    new "Der Dialogverlauf ist leer."

    # game/screens.rpy:998
    old "Keyboard"
    new "Tastatur"

    # game/screens.rpy:999
    old "Mouse"
    new "Maus"

    # game/screens.rpy:1002
    old "Gamepad"
    new "Gamepad"

    # game/screens.rpy:1015
    old "Enter"
    new "Enter"

    # game/screens.rpy:1016
    old "Advances dialogue and activates the interface."
    new "Führt den Dialog fort und aktiviert die Benutzeroberfläche."

    # game/screens.rpy:1019
    old "Space"
    new "Space"

    # game/screens.rpy:1020
    old "Advances dialogue without selecting choices."
    new "Führt den Dialog fort, ohne Entscheidungen auszuwählen."

    # game/screens.rpy:1023
    old "Arrow Keys"
    new "Pfeiltasten"

    # game/screens.rpy:1024
    old "Navigate the interface."
    new "Steuert die Benutzeroberfläche."

    # game/screens.rpy:1027
    old "Escape"
    new "ESC"

    # game/screens.rpy:1028
    old "Accesses the game menu."
    new "Ruft das Spielmenü auf."

    # game/screens.rpy:1031
    old "Ctrl"
    new "Strg"

    # game/screens.rpy:1032
    old "Skips dialogue while held down."
    new "Überspringt Dialog wenn gedrückt."

    # game/screens.rpy:1035
    old "Tab"
    new "Tab"

    # game/screens.rpy:1036
    old "Toggles dialogue skipping."
    new "Schaltet das Überspringen des Dialogs ein/aus."

    # game/screens.rpy:1039
    old "Page Up"
    new "Bild-Auf"

    # game/screens.rpy:1040
    old "Rolls back to earlier dialogue."
    new "Scrollt zurück zu früheren Dialog."

    # game/screens.rpy:1043
    old "Page Down"
    new "Bild-Ab"

    # game/screens.rpy:1044
    old "Rolls forward to later dialogue."
    new "Scrollt nach vorne zu späteren Dialog."

    # game/screens.rpy:1048
    old "Hides the user interface."
    new "Versteckt die Benutzeroberfläche."

    # game/screens.rpy:1052
    old "Takes a screenshot."
    new "Macht einen Screenshot."

    # game/screens.rpy:1056
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "Schaltet assistive {a=https://www.renpy.org/l/voicing}Sprachausgabe{/a} ein/aus."

    # game/screens.rpy:1062
    old "Left Click"
    new "Linksklick"

    # game/screens.rpy:1066
    old "Middle Click"
    new "Mittlere Maustaste"

    # game/screens.rpy:1070
    old "Right Click"
    new "Rechtsklick"

    # game/screens.rpy:1074
    old "Mouse Wheel Up\nClick Rollback Side"
    new "Mausrad nach oben\nKlick Rollback Seite"

    # game/screens.rpy:1078
    old "Mouse Wheel Down"
    new "Mausrad nach unten"

    # game/screens.rpy:1085
    old "Right Trigger\nA/Bottom Button"
    new "Rechter Trigger\nA/Untere Taste"

    # game/screens.rpy:1089
    old "Left Trigger\nLeft Shoulder"
    new "Linker Trigger\nLinke Schulter"

    # game/screens.rpy:1093
    old "Right Shoulder"
    new "Rechte Schulter"

    # game/screens.rpy:1098
    old "D-Pad, Sticks"
    new "D-Pad, Sticks"

    # game/screens.rpy:1102
    old "Start, Guide"
    new "Start, Guide"

    # game/screens.rpy:1106
    old "Y/Top Button"
    new "Y/Obere Taste"

    # game/screens.rpy:1109
    old "Calibrate"
    new "Kalibrieren"

    # game/screens.rpy:1174
    old "Yes"
    new "Ja"

    # game/screens.rpy:1175
    old "No"
    new "Nein"

    # game/screens.rpy:1221
    old "Skipping"
    new "Überspringen"

    # game/screens.rpy:1444
    old "Menu"
    new "Menü"
# TODO: Translation updated at 2020-04-25 19:42

translate deutsch strings:

    # game/screens.rpy:322
    old "Gallery"
    new "Galerie"
