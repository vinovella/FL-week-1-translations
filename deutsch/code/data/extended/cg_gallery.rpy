translate deutsch strings:

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 1: Stacy - Welcome home"
    new "Tag 1: Stacy - Willkommen zuhause"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Tell Stacy it's just for science"
    new "Sag Stacy dass es nur für die Wissenschaft ist"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 1: Stacy - Yoga training"
    new "Tag 1: Stacy - Yoga training"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Keep watching Stacy"
    new "Schau weiter zu Stacy"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 1: AmRose"
    new "Tag 1: AmRose"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Do NOT cum on Amrose"
    new "Nicht auf Amrose abspritzen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 2: Lydia"
    new "Tag 2: Lydia"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Don't post any request on FL or ignore any answer"
    new "Keine Anfrage auf FL posten oder irgendeine Antwort ignorieren"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 2: Polly - Morning"
    new "Tag 2: Polly - Morgen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Don't accept Polly's request"
    new "Polly's Bitte ablehnen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 2: Nora - Daylight"
    new "Tag 2: Nora - Mittags"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: Daisy - Morning"
    new "Tag 3: Daisy - Morgen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Ask Daisy to help you"
    new "Bitte Daisy dir zu helfen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: Lydia and friends"
    new "Tag 3: Lydia and friends"

    # game/code/data/extended/cg_gallery.rpy:15
    old "You must NOT romance Amrose"
    new "Du darfst AmRose NICHT umwerben"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: AmRose"
    new "Tag 3: AmRose"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Look at Amrose's smartphone"
    new "Schau dir AmRoses Handy an"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: Polly"
    new "Tag 3: Polly"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Disagree with Kevin's about cuckolding"
    new "Kevins Meinung nicht teilen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: AmRose and Maria"
    new "Tag 3: AmRose and Maria"

    # game/code/data/extended/cg_gallery.rpy:15
    old "You must have helped Kevin"
    new "Du musst Kevin geholfen haben"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: Daisy"
    new "Tag 3: Daisy"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Refuse Dahlia's proposal"
    new "Dahlia's Vorschlag ablehnen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 3: Dahlia"
    new "Tag 3: Dahlia"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Accept Dahlia's proposal but don't obey to her"
    new "Akzeptiere Dahlias Vorschlag aber gehorche ihr nicht"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: The guests"
    new "Die Party: Die Gäste"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: Allison"
    new "Die party: Allison"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: Lyssa"
    new "Die party: Lyssa"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Choose Secret Santa and offer Lyssa to lick her"
    new "Wähle Secret Santa und biete Lyssa an, sie zu lecken"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: Hana"
    new "Die party: Hana"

    # game/code/data/extended/cg_gallery.rpy:15
    old "You must have peeked on Hana on day 1"
    new "Du musst Hana an Tag 1 beobachtet haben"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: Lydia"
    new "Die party: Lydia"

    # game/code/data/extended/cg_gallery.rpy:15
    old "The party: Ending"
    new "Die Party: Endeg"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 4: Breakfast at Min's"
    new "Tag 4: Frühstück bei Min"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 4: Stacy - Morning"
    new "Tag 4: Stacy - Morgen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Ask Stacy more about her diet"
    new "Frag Stacy weiter nach ihrer Diät"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 4: First date... or not?"
    new "Tag 4: Erstes Date ... oder nicht?"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 5: AmRose and Maria"
    new "Tag 5: AmRose and Maria"

    # game/code/data/extended/cg_gallery.rpy:15
    old "You must be into watersports or have joined Antony's event"
    new "Entweder sie mögen Wassersport oder sie haben an Antony's Event teilgenommen."

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 5: AmRose"
    new "Tag 5: AmRose"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 5: Vanessa and Lyssa"
    new "Tag 5: Vanessa and Lyssa"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Welcome, Stacy!"
    new "Tag 6: Willkommen Stacy!"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Stacy & AmRose - Lunch"
    new "Tag 6: Stacy & AmRose - Mittagessen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Before the concert"
    new "Tag 6: Vor dem Konzert"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Maria"
    new "Tag 6: Maria"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Punish Maria and ask her for pictures"
    new "Bestrafe Maria und frag sie nach Bildern"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: After the concert"
    new "Tag 6: Nach dem Konzert"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Lyssa"
    new "Tag 6: Lyssa"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Be a nice guy"
    new "Sei ein netter Kerl"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 6: Stacy - The dorm"
    new "Tag 6: Stacy - Der Schlafsaal"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: A (not so) good morning"
    new "Tag 7: Ein (nicht so) guten Morgen"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: So, what's FL?"
    new "Tag 7: Also, was ist FL?"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: Life's a beach"
    new "Tag 7: Life's a beach"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: Daisy & Dahlia"
    new "Tag 7: Daisy & Dahlia"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: Vanessa"
    new "Tag 7: Vanessa"

    # game/code/data/extended/cg_gallery.rpy:15
    old "Day 7: Evening"
    new "Tag 7: Abend"

translate deutsch strings:

    # game/code/data/extended/cg_gallery.rpy:21
    old "Day 1: Mysterious girl"
    new "Tag 1: Geheimnisvolles Mädchen"

    # game/code/data/extended/cg_gallery.rpy:21
    old "Don't visit Amrose"
    new "Besuche AmRose nicht"

    # game/code/data/extended/cg_gallery.rpy:21
    old "The party: Mysterious girl"
    new "The party: Geheimnisvolles Mädchen"

    # game/code/data/extended/cg_gallery.rpy:21
    old "Day 6: Karen (flashback)"
    new "Tag 6: Karen (Rückblende)"

    # game/code/data/extended/cg_gallery.rpy:21
    old "Day 6: Jessie (park)"
    new "Tag 6: Jessie (park)"

translate deutsch strings:

    # game/code/data/extended/cg_gallery.rpy:21
    old "You must have helped Kevin and respected AmRose privacy"
    new "Du musst Kevin geholfen haben und AmRose's Privatsphäre respektiert haben"

