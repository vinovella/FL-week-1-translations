translate deutsch strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Min & Antony"
    new "Tag 1: Min & Antony"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to cumshots"
    new "Sag ja zu Cumshots"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Chloe & Peter"
    new "Tag 1: Chloe & Peter"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Stacy (flashback)"
    new "Tag 2: Stacy (Rückblende)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Nora & Polly"
    new "Tag 2: Nora & Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "You must have posted your lips, met Polly and accepted her conditions"
    new "Du musst deine Lippen gepostet, Polly getroffen und ihre Bedingungen akzeptiert haben"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Min"
    new "Tag 2: Min"

    # game/code/data/extended/replay_room.rpy:15
    old "Make Min cum and keep watching her until the end"
    new "Lass Min kommen und beobachte sie bis zum Ende"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - Nora route"
    new "Tag 2: Polly - Nora route"

    # game/code/data/extended/replay_room.rpy:15
    old "Assfinger Polly"
    new "Assfinger Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - solo"
    new "Tag 2: Polly - solo"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Wird freigeschaltet, indem keine Anfrage auf FL gestellt oder eine Antwort ignoriert wird"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Stacy (flashback)"
    new "Tag 3: Stacy (Rückblende)"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't scare Stacy"
    new "Erschreck Stacy nicht"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Chloe (NTR)"
    new "Tag 3: Chloe (NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Save Kevin"
    new "Rette Kevin"

    # game/code/data/extended/replay_room.rpy:15
    old "Submit to Dahlia"
    new "Bei Dahlia einreichen"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Secret Santa"
    new "Party: Secret Santa"

    # game/code/data/extended/replay_room.rpy:15
    old "Choose the event and request to see Lyssa's pussy"
    new "Wähle ein Event und verlange Lyssa's pussy zu sehen"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: QueenB"
    new "Party: QueenB"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle's event"
    new "Party: Squizzle's Veranstaltung"

    # game/code/data/extended/replay_room.rpy:15
    old "Do whatever it takes to win"
    new "Tu alles, um zu gewinnen"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Mr.D1cky's event"
    new "Party: Mr.D1cky's Veranstaltung"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Allison & Vesna"
    new "Party: Allison & Vesna"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle & Mr.D1cky"
    new "Party: Squizzle & Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Daisy"
    new "Tag 4: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Command Daisy to blow you (you must have refused Dahlia's request)"
    new "Befiehl Daisy, dir einen zu blasen (Sie müssen Dahlias Bitte abgelehnt haben)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: AmRose"
    new "Tag 4: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to enema play"
    new "Sag ja zum Enemaspiel"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Polly"
    new "Tag 4: Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Polly & Nora"
    new "Tag 4: Polly & Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Finger Polly (any hole)"
    new "Finger Polly (jedes Loch)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Chloe"
    new "Tag 5: Chloe"

    # game/code/data/extended/replay_room.rpy:15
    old "It's showtime! (available if you have supported Kevin's views on NTR)"
    new "Es ist Showtime! (verfügbar, wenn du Kevins Ansichten zu NTR unterstützt hast)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Polly"
    new "Tag 5: Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Strip and join Polly"
    new "Zieh dich aus und mach bei Polly mit"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URCumSlut"
    new "Tag 5: URCumSlut"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Squizzle"
    new "Tag 5: Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Earn a gold star"
    new "Verdiene dir einen goldenen Stern"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Daisy"
    new "Tag 5: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Invite Daisy and don't be shy"
    new "Lade Daisy ein und sei nicht schüchtern"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Stacy (flashback)"
    new "Tag 5: Stacy (Rückblende)"

    # game/code/data/extended/replay_room.rpy:15
    old "Jerk off"
    new "Hol dir einen runter"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Daisy"
    new "Tag 6: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Daisy"
    new "Creampie Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Lydia"
    new "Tag 6: Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Nora"
    new "Tag 6: Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Nora"
    new "Creampie Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Polly & Nora"
    new "Tag 6: Polly & Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked if you are not romancing Polly"
    new "Wird freigeschaltet wenn du nicht mit Polly flirtest"

    # game/code/data/extended/replay_room.rpy:15
    old "Take a more dominant role"
    new "Nimm eine dominantere Rolle ein"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Stacy"
    new "Tag 6: Stacy"

    # game/code/data/extended/replay_room.rpy:15
    old "Cum on Vanessa's feet"
    new "Komm auf Vanessas Füßen"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Hana"
    new "Tag 7: Hana"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask for a release"
    new "Bitte um eine Erleichterung"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Daisy"
    new "Tag 7: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Dahlia"
    new "Tag 7: Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Lyssa"
    new "Tag 7: Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Spank Lyssa twelve times"
    new "Verhaue Lyssa zwölf Mal"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: AmRose"
    new "Tag 7: AmRose"

translate deutsch strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Jessie (dorm)"
    new "Tag 6: Jessie (Wohnheim)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Maria & AmRose (flashback)"
    new "Tag 7: Maria & AmRose (Rückblende)"

    # game/code/data/extended/replay_room.rpy:15
    old "Enjoy the rimjob"
    new "Genieße den Rimjob"

translate deutsch strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URLittlePony"
    new "Tag 5: URLittlePony"

