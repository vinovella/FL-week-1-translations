translate dutch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Lydia"
    new "Day 1: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ignore the girl Pete is sleeping with (name's Hana)"
    new "Ignore the girl Pete is sleeping with (name's Hana)"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "Day 1: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "Day 2: Squizzle's message"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Did you post your lips in the morning?"
    new "Did you post your lips in the morning?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Day 2: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "Day 2: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlocked by not posting any request on FL or by ignoring the answer"
    new "Unlocked by not posting any request on FL or by ignoring the answer"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "Day 3: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "You must have made Min cum on day 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Party: Daisy and Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Choose the Secret Oral Santa event"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Party: Mysterious girl"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "Day 4: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "Day 4: AmRose's bathtub"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Polly"
    new "Day 4: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "Day 5: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "Day 5: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "You must have discovered Lyssa's secret"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "Day 5: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "Day 5: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Give Min private lessons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 patrons"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Stacy"
    new "Day 6: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "Day 6: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Nora"
    new "Day 6: Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Don't creampie Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "Day 6: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "You must have chosen Secret Oral Santa but you don't know her secret yet"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "Day 6: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "You have seen all the possible pictures that Maria can send"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "Day 7: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "Day 7: beach"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Hana"
    new "Day 7: Hana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Don't ask for a release"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Daisy"
    new "Day 7: Daisy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Lyssa"
    new "Day 7: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "Day 7: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: AmRose"
    new "Day 7: AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Stacy in hell"
    new "Stacy in hell"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Disappoint Stacy"
    new "Disappoint Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Stacy"
    new "Halloween Bonus - Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Special"
    new "Halloween Special"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Jessie"
    new "Halloween Bonus - Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Halloween Bonus - Samiya"
    new "Halloween Bonus - Samiya"

