translate dutch strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Min & Antony"
    new "Day 1: Min & Antony"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to cumshots"
    new "Say yes to cumshots"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 1: Chloe & Peter"
    new "Day 1: Chloe & Peter"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Stacy (flashback)"
    new "Day 2: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Nora & Polly"
    new "Day 2: Nora & Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "You must have posted your lips, met Polly and accepted her conditions"
    new "You must have posted your lips, met Polly and accepted her conditions"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Min"
    new "Day 2: Min"

    # game/code/data/extended/replay_room.rpy:15
    old "Make Min cum and keep watching her until the end"
    new "Make Min cum and keep watching her until the end"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - Nora route"
    new "Day 2: Polly - Nora route"

    # game/code/data/extended/replay_room.rpy:15
    old "Assfinger Polly"
    new "Assfinger Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 2: Polly - solo"
    new "Day 2: Polly - solo"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Stacy (flashback)"
    new "Day 3: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't scare Stacy"
    new "Don't scare Stacy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 3: Chloe (NTR)"
    new "Day 3: Chloe (NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Save Kevin"
    new "Save Kevin"

    # game/code/data/extended/replay_room.rpy:15
    old "Submit to Dahlia"
    new "Submit to Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Secret Santa"
    new "Party: Secret Santa"

    # game/code/data/extended/replay_room.rpy:15
    old "Choose the event and request to see Lyssa's pussy"
    new "Choose the event and request to see Lyssa's pussy"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: QueenB"
    new "Party: QueenB"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle's event"
    new "Party: Squizzle's event"

    # game/code/data/extended/replay_room.rpy:15
    old "Do whatever it takes to win"
    new "Do whatever it takes to win"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Mr.D1cky's event"
    new "Party: Mr.D1cky's event"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Allison & Vesna"
    new "Party: Allison & Vesna"

    # game/code/data/extended/replay_room.rpy:15
    old "Party: Squizzle & Mr.D1cky"
    new "Party: Squizzle & Mr.D1cky"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Daisy"
    new "Day 4: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Command Daisy to blow you (you must have refused Dahlia's request)"
    new "Command Daisy to blow you (you must have refused Dahlia's request)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: AmRose"
    new "Day 4: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Say yes to enema play"
    new "Say yes to enema play"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 4: Polly & Nora"
    new "Day 4: Polly & Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Finger Polly (any hole)"
    new "Finger Polly (any hole)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Chloe"
    new "Day 5: Chloe"

    # game/code/data/extended/replay_room.rpy:15
    old "It's showtime! (available if you have supported Kevin's views on NTR)"
    new "It's showtime! (available if you have supported Kevin's views on NTR)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Polly"
    new "Day 5: Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Strip and join Polly"
    new "Strip and join Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: URLittlePony"
    new "Day 5: URLittlePony"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Squizzle"
    new "Day 5: Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Earn a gold star"
    new "Earn a gold star"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Daisy"
    new "Day 5: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Invite Daisy and don't be shy"
    new "Invite Daisy and don't be shy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 5: Stacy (flashback)"
    new "Day 5: Stacy (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Jerk off"
    new "Jerk off"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Daisy"
    new "Day 6: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Daisy"
    new "Creampie Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Jessie (dorm)"
    new "Day 6: Jessie (dorm)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Lydia"
    new "Day 6: Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Creampie Nora"
    new "Creampie Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 6: Polly & Nora"
    new "Day 6: Polly & Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Unlocked if you are not romancing Polly"
    new "Unlocked if you are not romancing Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Take a more dominant role"
    new "Take a more dominant role"

    # game/code/data/extended/replay_room.rpy:15
    old "Cum on Vanessa's feet"
    new "Cum on Vanessa's feet"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask for a release"
    new "Ask for a release"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Maria & AmRose (flashback)"
    new "Day 7: Maria & AmRose (flashback)"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 7: Dahlia"
    new "Day 7: Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Spank Lyssa twelve times"
    new "Spank Lyssa twelve times"

    # game/code/data/extended/replay_room.rpy:15
    old "Enjoy the rimjob"
    new "Enjoy the rimjob"

